...
angular.module('ui.grid').directive('uiGridCell',
    ['$compile', '$parse', 'gridUtil', 'uiGridConstants',
      function ($compile, $parse, gridUtil, uiGridConstants) {
        var uiGridCell = {
          priority: 0,
          scope: false,
          require: '?^uiGrid',
              ...
              pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                ...
                // No controller, compile the element manually (for unit tests)
                  ...
                  if (uiGridCtrl && !$scope.col.compiledElementFn) {
                    ...
                    .replace(uiGridConstants.MODEL_COL_FIELD,
                        'row.entity.' + gridUtil.preEval($scope.col.field))
                    .replace(uiGridConstants.COL_FIELD,
                ...
                var updateClass = function (grid) {
                  ...
                  if (angular.isFunction($scope.col.cellClass)) {
                    classAdded = $scope.col.cellClass($scope.grid, $scope.row,
                        $scope.col, $scope.rowRenderIndex,
...
(function () {
  ...
  .service('uiGridColumnMenuService',
      ['i18nService', 'uiGridConstants', 'gridUtil',
        function (i18nService, uiGridConstants, gridUtil) {
          ...
          var service = {
            ...
            setColMenuItemWatch: function ($scope) {
              var deregFunction = $scope.$watch('col.menuItems',
                  function (n, o) {
            ...
            isActiveSort: function ($scope, direction) {
              return (typeof ($scope.col) !== 'undefined'
            ...
            suppressRemoveSort: function ($scope) {
              if ($scope.col && $scope.col.colDef
                  && $scope.col.colDef.suppressRemoveSort) {
                return true;
              } else {
                return false;
              }
            ...
            hideable: function ($scope) {
              if (typeof ($scope.col) !== 'undefined' && $scope.col
                  && $scope.col.colDef && $scope.col.colDef.enableHiding
                  === false) {
                return false;
              } else {
                return true;
             ...
             *
             */
            getColumnElementPosition: function ($scope, column,
                $columnElement) {
              var positionData = {};
              ...
              positionData.height = gridUtil.elementHeight($columnElement,
                  true);
            ...
            repositionMenu: function ($scope, column, positionData, $elm,
              ...
              var containerId = column.renderContainer ? column.renderContainer
                  : 'body';
              ...
              var renderContainerOffset = renderContainerElm.getBoundingClientRect().left
                  - $scope.grid.element[0].getBoundingClientRect().left;

              var containerScrollLeft = renderContainerElm.querySelectorAll(
                  '.ui-grid-viewport')[0].scrollLeft;
              ...
              if (menu.length !== 0) {
                ...
                if (mid.length !== 0 && !angular.element(mid).hasClass(
                    'ng-hide')) {
                  ...
                  paddingRight = parseInt(gridUtil.getStyles(
                      angular.element(menu)[0])['paddingRight'], 10);
              ...
              var left = positionData.left + renderContainerOffset
                  - containerScrollLeft + positionData.width - myWidth
                  + paddingRight;
  ...
  .directive('uiGridColumnMenu',
      ['$timeout', 'gridUtil', 'uiGridConstants', 'uiGridColumnMenuService',
        function ($timeout, gridUtil, uiGridConstants,
          ...
          var uiGridColumnMenu = {
            ...
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
              ...
              $scope.showMenu = function (column, $columnElement, event) {
                ...
                var colElementPosition = uiGridColumnMenuService.getColumnElementPosition(
                    $scope, column, $columnElement);
...
(function () {
  ...
  angular.module('ui.grid').directive('uiGridFooterCell',
      ['$timeout', 'gridUtil', 'uiGridConstants', '$compile',
        function ($timeout, gridUtil, uiGridConstants, $compile) {
          var uiGridFooterCell = {
            ...
            compile: function compile(tElement, tAttrs, transclude) {
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  var cellFooter = $compile($scope.col.footerCellTemplate)(
                      $scope);
                  ...
                  var updateClass = function (grid) {
                    ...
                    if (angular.isFunction($scope.col.footerCellClass)) {
                      classAdded = $scope.col.footerCellClass($scope.grid,
                          $scope.row, $scope.col, $scope.rowRenderIndex,
                          $scope.colRenderIndex);
                    } else {
              ...
              return {
                pre: function ($scope, $elm, $attrs, controllers) {
                  ...
                  var footerTemplate = ($scope.grid.options.footerTemplate)
                      ? $scope.grid.options.footerTemplate : defaultTemplate;
                  ...
                  .then(function (contents) {
                    ...
                    if (containerCtrl) {
                      ...
                      var footerViewport = $elm[0].getElementsByClassName(
                          'ui-grid-footer-viewport')[0];
              ...
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  var footerTemplate = ($scope.grid.options.gridFooterTemplate)
                      ? $scope.grid.options.gridFooterTemplate
                      : defaultTemplate;
                  gridUtil.getTemplate(footerTemplate)
                  .then(function (contents) {
                    var template = angular.element(contents);
...
(function () {
  ...
  angular.module('ui.grid').directive('uiGridGroupPanel',
      ["$compile", "uiGridConstants", "gridUtil",
        function ($compile, uiGridConstants, gridUtil) {
...
(function () {
  ...
  angular.module('ui.grid').directive('uiGridHeaderCell',
      ['$compile', '$timeout', '$window', '$document', 'gridUtil',
        'uiGridConstants', 'ScrollEvent',
        function ($compile, $timeout, $window, $document, gridUtil,
            uiGridConstants, ScrollEvent) {
            ...
            compile: function () {
              return {
                pre: function ($scope, $elm, $attrs) {
                  var cellHeader = $compile($scope.col.headerCellTemplate)(
                      $scope);
                ...
                post: function ($scope, $elm, $attrs, controllers) {
                  ...
                  var updateClass = function (grid) {
                    ...
                    $scope.isLastCol = ($scope.col
                        === rightMostContainer.visibleColumnCache[rightMostContainer.visibleColumnCache.length
                        - 1]);
                  };
                  ...
                  if ($scope.sortable || $scope.colMenu) {
                    ...
                    var downEvent = gridUtil.isTouchEnabled() ? 'touchstart'
                        : 'mousedown';
                    $contentsElm.on(downEvent, function (event) {
                      ...
                      uiGridCtrl.fireEvent(
                          uiGridConstants.events.COLUMN_HEADER_CLICK,
                          {event: event, columnName: $scope.col.colDef.name});
                    });

                    var upEvent = gridUtil.isTouchEnabled() ? 'touchend'
                        : 'mouseup';
                  ...
                  if ($scope.sortable) {
                    var clickEvent = gridUtil.isTouchEnabled() ? 'touchend'
                        : 'click';
                  ...
                  if ($scope.filterable) {
                    ...
                    angular.forEach($scope.col.filters, function (filter, i) {
                      filterDeregisters.push(
                          $scope.$watch('col.filters[' + i + '].term',
                              function (n, o) {
                                if (n !== o) {
                                  uiGridCtrl.grid.api.core.raise.filterChanged();
                                  uiGridCtrl.grid.refresh()
                                  .then(function () {
                                    if (uiGridCtrl.prevScrollArgs
                                        && uiGridCtrl.prevScrollArgs.y
                                        && uiGridCtrl.prevScrollArgs.y.percentage) {
                                      var scrollEvent = new ScrollEvent(
                                          uiGridCtrl.grid, null, null,
                                          'uiGridHeaderCell.toggleMenu');
                    ...
                    $scope.$on('$destroy', function () {
                      angular.forEach(filterDeregisters,
                          function (filterDeregister) {
                            filterDeregister();
                          });
                    });
                  }
                ...
                pre: function ($scope, $elm, $attrs, controllers) {
                  ...
                  .then(function (contents) {
                    ...
                    if (containerCtrl) {
                      ...
                      var headerViewport = $elm[0].getElementsByClassName(
                  ...
                  function updateColumnWidths() {
                    ...
                    var getColWidth = function (column) {
                      if (column.widthType === "manual") {
                        ...
                        return parseInt(column.width.replace(/%/g, ''), 10)
                            * availableWidth / 100;
                        ...
                        if (oneAsterisk === 0) {
                          oneAsterisk = parseInt(leftoverWidth / asteriskNum,
                              10);
                        }
                    ...
                    columnCache.forEach(function (column) {
                      ...
                      } else if (angular.isString(column.width)
                    ...
                    columnCache.filter(function (column) {
                      ...
                      return colWidthPriority.indexOf(a.widthType)
                          - colWidthPriority.indexOf(b.widthType);
                    ...
                    if (hasVariableWidth && leftoverWidth > 0 && canvasWidth > 0
                        && canvasWidth < availableWidth) {
                      var remFn = function (column) {
                        if (leftoverWidth > 0 && (column.widthType === "auto"
                            || column.widthType === "percent")) {
                      ...
                      } while (leftoverWidth > 0 && leftoverWidth
                      !== prevLeftover);
                    }
                    ...
                    if (columnCache.length > 0) { // && grid.options.enableGridMenu) {
                      columnCache[columnCache.length
                      - 1].headerWidth = columnCache[columnCache.length
                      - 1].drawnWidth - 30;
                    }

                    containerCtrl.colContainer.canvasWidth = parseInt(
                        canvasWidth, 10);
...
(function () {
  ...
  .service('uiGridGridMenuService',
      ['gridUtil', 'i18nService', 'uiGridConstants',
        function (gridUtil, i18nService, uiGridConstants) {
          /**
           *  @ngdoc service
          ...
          var service = {
            ...
            initialize: function ($scope, grid) {
               ...
               *
               */
              grid.api.registerMethod('core', 'addToGridMenu',
                  service.addToGridMenu);
            ...
            addToGridMenu: function (grid, menuItems) {
              if (!angular.isArray(menuItems)) {
                ...
                if (grid.gridMenuScope) {
                  ...
                  grid.gridMenuScope.registeredMenuItems = grid.gridMenuScope.registeredMenuItems.concat(
                      menuItems);
                } else {
                  gridUtil.logError(
            ...
            removeFromGridMenu: function (grid, id) {
              ...
              if (grid && grid.gridMenuScope) {
                grid.gridMenuScope.registeredMenuItems.forEach(
            ...
            showHideColumns: function ($scope) {
              ...
              if (!$scope.grid.options.columnDefs
                  || $scope.grid.options.columnDefs.length === 0
                  || $scope.grid.columns.length === 0) {
              ...
              $scope.grid.options.gridMenuTitleFilter = $scope.grid.options.gridMenuTitleFilter
                  ? $scope.grid.options.gridMenuTitleFilter : function (title) {
                    return title;
                  };
            ...
            setMenuItemTitle: function (menuItem, colDef, grid) {
              var title = grid.options.gridMenuTitleFilter(
                  colDef.displayName || colDef.name || colDef.field);
  ...
  .directive('uiGridMenuButton',
      ['gridUtil', 'uiGridConstants', 'uiGridGridMenuService',
...
(function () {
      ...
      ['$compile', '$timeout', '$window', '$document', 'gridUtil',
        ...
        function ($compile, $timeout, $window, $document, gridUtil,
           ...
           */
              ...
              if (uiGridCtrl) {
                $scope.$on('$destroy',
                    uiGridCtrl.grid.api.core.on.scrollEvent($scope,
                        applyHideMenu));
              }

              $scope.$on('$destroy',
                  $scope.$on(uiGridConstants.events.ITEM_DRAGGING,
                      applyHideMenu));
            },
  ...
  .directive('uiGridMenuItem', ['gridUtil', '$compile', 'i18nService',
    function (gridUtil, $compile, i18nService) {
      var uiGridMenuItem = {
        priority: 0,
        scope: {
          name: '=',
...
(function () {
  ...
  module.directive('uiGridRenderContainer',
      ['$timeout', '$document', 'uiGridConstants', 'gridUtil', 'ScrollEvent',
              ...
              return {
                pre: function prelink($scope, $elm, $attrs, controllers) {
                  ...
                  if (!grid.renderContainers[$scope.rowContainerName]) {
                    throw "Row render container '" + $scope.rowContainerName
                    + "' is not registered.";
                  }
                  if (!grid.renderContainers[$scope.colContainerName]) {
                    throw "Column render container '" + $scope.colContainerName
                    + "' is not registered.";
                  }
                ...
                post: function postlink($scope, $elm, $attrs, controllers) {
                  ...
                  $elm.addClass(
                      'ui-grid-render-container-' + $scope.containerId);

                  // Bind to left/right-scroll events
                  if ($scope.bindScrollHorizontal
                      || $scope.bindScrollVertical) {
                  ...
                  function scrollHandler(args) {
                    ...
                    if (args.y && $scope.bindScrollVertical) {
                      ...
                      var newScrollTop = args.getNewScrollTop(rowContainer,
                          containerCtrl.viewport);
                    ...
                    if (args.x && $scope.bindScrollHorizontal) {
                      containerCtrl.prevScrollArgs = args;
                      var newScrollLeft = args.getNewScrollLeft(colContainer,
                          containerCtrl.viewport);
                      ...
                      if (containerCtrl.headerViewport) {
                        containerCtrl.headerViewport.scrollLeft = gridUtil.denormalizeScrollLeft(
                            containerCtrl.headerViewport, newScrollLeft);
                      }

                      if (containerCtrl.footerViewport) {
                        containerCtrl.footerViewport.scrollLeft = gridUtil.denormalizeScrollLeft(
                            containerCtrl.footerViewport, newScrollLeft);
                      }
                  ...
                  $elm.bind(
                      ...
                      function (evt) {
                        ...
                        var scrollEvent = new ScrollEvent(grid, rowContainer,
                            colContainer,
                            ScrollEvent.Sources.RenderContainerMouseWheel);
                        if (newEvent.deltaY !== 0) {
                          var scrollYAmount = newEvent.deltaY * -120;

                          // Get the scroll percentage
                          var scrollYPercentage = (containerCtrl.viewport[0].scrollTop
                              + scrollYAmount)
                              / rowContainer.getVerticalScrollLength();
                        ...
                        if (newEvent.deltaX !== 0) {
                          ...
                          // Get the scroll percentage
                          var scrollLeft = gridUtil.normalizeScrollLeft(
                              containerCtrl.viewport);
                          var scrollXPercentage = (scrollLeft + scrollXAmount)
                              / (colContainer.getCanvasWidth()
                                  - colContainer.getViewportWidth());
                        ...
                        if ((scrollEvent.y && scrollEvent.y.percentage !== 0
                            && scrollEvent.y.percentage !== 1
                            && containerCtrl.viewport[0].scrollTop !== 0) ||
                            (scrollEvent.x && scrollEvent.x.percentage !== 0
                                && scrollEvent.x.percentage !== 1)) {
                          evt.preventDefault();
                        }
                  ...
                  function touchmove(event) {
                    ...
                    var scrollEvent = new ScrollEvent(grid, rowContainer,
                        colContainer,
                        ScrollEvent.Sources.RenderContainerTouchMove);

                    if (deltaY !== 0) {
                      var scrollYPercentage = (scrollTopStart + deltaY)
                          / rowContainer.getVerticalScrollLength();
                    ...
                    if (deltaX !== 0) {
                      var scrollXPercentage = (scrollLeftStart + deltaX)
                          / (colContainer.getCanvasWidth()
                              - colContainer.getViewportWidth());
                  ...
                  $elm.bind('$destroy', function () {
                    ...
                    ['touchstart', 'touchmove', 'touchend', 'keydown', 'wheel',
                  ...
                  function update() {
                    ...
                    ret += '\n .grid' + uiGridCtrl.grid.id
                        + ' .ui-grid-render-container-' + $scope.containerId
                        + ' .ui-grid-canvas { width: ' + canvasWidth
                        + 'px; height: ' + canvasHeight + 'px; }';

                    ret += '\n .grid' + uiGridCtrl.grid.id
                        + ' .ui-grid-render-container-' + $scope.containerId
                        + ' .ui-grid-header-canvas { width: ' + (canvasWidth
                            + grid.scrollbarWidth) + 'px; }';

                    ret += '\n .grid' + uiGridCtrl.grid.id
                        + ' .ui-grid-render-container-' + $scope.containerId
                        + ' .ui-grid-viewport { width: ' + viewportWidth
                        + 'px; height: ' + viewportHeight + 'px; }';
                    ret += '\n .grid' + uiGridCtrl.grid.id
                        + ' .ui-grid-render-container-' + $scope.containerId
                        + ' .ui-grid-header-viewport { width: '
                        + headerViewportWidth + 'px; }';

                    ret += '\n .grid' + uiGridCtrl.grid.id
                        + ' .ui-grid-render-container-' + $scope.containerId
                        + ' .ui-grid-footer-canvas { width: ' + canvasWidth
                        + grid.scrollbarWidth + 'px; }';
                    ret += '\n .grid' + uiGridCtrl.grid.id
                        + ' .ui-grid-render-container-' + $scope.containerId
                        + ' .ui-grid-footer-viewport { width: '
                        + footerViewportWidth + 'px; }';
                    ...
                    if (renderContainer.explicitHeaderHeight !== undefined
                        && renderContainer.explicitHeaderHeight !== null
                        && renderContainer.explicitHeaderHeight > 0) {
                      ret += '\n .grid' + uiGridCtrl.grid.id
                          + ' .ui-grid-render-container-' + $scope.containerId
                          + ' .ui-grid-header-cell { height: '
                          + renderContainer.explicitHeaderHeight + 'px; }';
                    }
                    ...
                    else if (renderContainer.innerHeaderHeight !== undefined
                        && renderContainer.innerHeaderHeight !== null
                        && renderContainer.innerHeaderHeight > 0) {
                      ret += '\n .grid' + uiGridCtrl.grid.id
                          + ' .ui-grid-render-container-' + $scope.containerId
                          + ' .ui-grid-header-cell { height: '
                          + renderContainer.innerHeaderHeight + 'px; }';
                    }
  ...
  module.controller('uiGridRenderContainer',
      ['$scope', 'gridUtil', function ($scope, gridUtil) {
        var self = this;
        ...
        self.rowStyle = function (index) {
          ...
          if (!renderContainer.disableColumnOffset
              && $scope.colContainer.currentFirstColumn !== 0) {
            if ($scope.grid.isRTL()) {
          ...
          compile: function () {
            return {
              pre: function ($scope, $elm, $attrs, controllers) {
                ...
                function compileTemplate() {
                  $scope.row.getRowTemplateFn.then(
                      function (compiledElementFn) {
                ...
                $scope.$watch('row.getRowTemplateFn',
...
// 'use strict';
  ...
  angular.module('ui.grid').directive('uiGridStyle',
      ['gridUtil', '$interpolate', function (gridUtil, $interpolate) {
        return {
          // restrict: 'A',
          // priority: 1000,
          // require: '?^uiGrid',
...
(function () {
  ...
  angular.module('ui.grid').directive('uiGridViewport',
      ['gridUtil', 'ScrollEvent', 'uiGridConstants',
            ...
            link: function ($scope, $elm, $attrs, controllers) {
              ...
              $elm.on('scroll', function (evt) {
                ...
                if (newScrollLeft !== colContainer.prevScrollLeft) {
                  ...
                  var horizScrollLength = (colContainer.getCanvasWidth()
                      - colContainer.getViewportWidth());
                  ...
                  colContainer.adjustScrollHorizontal(newScrollLeft,
                      horizScrollPercentage);
                }
                ...
                if (newScrollTop !== rowContainer.prevScrollTop) {
                  ...
                  rowContainer.adjustScrollVertical(newScrollTop,
                      vertScrollPercentage);
                }

                var scrollEvent = new ScrollEvent(grid, rowContainer,
                    colContainer, ScrollEvent.Sources.ViewPortScroll);
      ...
      ['$scope', '$element', '$attrs', 'gridUtil', '$q', 'uiGridConstants',
        ...
        function ($scope, $elm, $attrs, gridUtil, $q, uiGridConstants,
          ...
          if (angular.isString($scope.uiGrid.data)) {
            dataWatchCollectionDereg = $scope.$parent.$watchCollection(
                $scope.uiGrid.data, dataWatchFunction);
          } else {
            dataWatchCollectionDereg = $scope.$parent.$watchCollection(
                function () {
          ...
          var columnDefWatchCollectionDereg = $scope.$parent.$watchCollection(
              function () {
          ...
          function columnDefsWatchFunction(n, o) {
            if (n && n !== o) {
              ...
              .then(function () {
                ...
                self.grid.callDataChangeCallbacks(
                    uiGridConstants.dataChange.COLUMN);
              });
            }
          }
          ...
          function dataWatchFunction(newData) {
            ...
            if (newData) {
              if (
                  ...
                  self.grid.columns.length === (self.grid.rowHeaderColumns
                  ? self.grid.rowHeaderColumns.length : 0) &&
              ...
              if (self.grid.options.columnDefs.length > 0 || newData.length
                  > 0) {
                  ...
                  $scope.$evalAsync(function () {
                    ...
                    $timeout(function () {
                      ...
                      if (self.grid.options.enableInfiniteScroll) {
                        ...
                        if (self.grid.renderContainers.body.prevRowScrollIndex
                            === 0) {
                          adjustInfiniteScrollPosition(0);
                        }
                        //If we are scrolling up, we need to reseed the grid.
                        if (self.grid.scrollDirection
                            === uiGridConstants.scrollDirection.UP) {
                          adjustInfiniteScrollPosition(
                              self.grid.renderContainers.body.prevRowScrollIndex
                              + 1 + self.grid.options.excessRows);
                        }
                      }
                    }, 0);
                  });
            ...
            compile: function () {
              return {
                post: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  grid.gridWidth = $scope.gridWidth = gridUtil.elementWidth(
                      $elm);

                  // Default canvasWidth to the grid width, in case we don't get any column definitions to calculate it from
                  grid.canvasWidth = uiGridCtrl.grid.gridWidth;

                  grid.gridHeight = $scope.gridHeight = gridUtil.elementHeight(
                      $elm);
                  ...
                  if (grid.gridHeight < grid.options.rowHeight) {
                    ...
                    var headerHeight = grid.options.showHeader
                        ? grid.options.headerRowHeight : 0;
                    var footerHeight = grid.calcFooterHeight();

                    var scrollbarHeight = 0;
                    if (grid.options.enableHorizontalScrollbar
                        === uiGridConstants.scrollbars.ALWAYS) {
                    ...
                    var newHeight = headerHeight + contentHeight + footerHeight
              ...
              post: function ($scope, $elm, $attrs, uiGridCtrl) {
                ...
                function updateContainerDimensions() {
                  ...
                  if ($scope.side === 'left' || $scope.side === 'right') {
                    ...
                    ret += '.grid' + grid.id + ' .ui-grid-pinned-container-'
                        + $scope.side + ', .grid' + grid.id
                        + ' .ui-grid-pinned-container-' + $scope.side
                        + ' .ui-grid-render-container-' + $scope.side
                        + ' .ui-grid-viewport { width: ' + myWidth
                        + 'px; height: ' + myHeight + 'px; } ';
                  }
...
(function () {
  ...
  angular.module('ui.grid')
  .factory('Grid',
      ['$q', '$compile', '$parse', 'gridUtil', 'uiGridConstants', 'GridOptions',
        'GridColumn', 'GridRow', 'GridApi', 'rowSorter', 'rowSearcher',
        'GridRenderContainer', '$timeout',
        function ($q, $compile, $parse, gridUtil, uiGridConstants, GridOptions,
          ...
          var Grid = function Grid(options) {
            ...
            if (options !== undefined && typeof (options.id) !== 'undefined'
                && options.id) {
              if (!/^[_a-zA-Z0-9-]+$/.test(options.id)) {
                throw new Error("Grid id '" + options.id
            ...
            if (self.options.enableHorizontalScrollbar
                === uiGridConstants.scrollbars.ALWAYS) {
              self.scrollbarHeight = gridUtil.getScrollbarWidth();
            }

            if (self.options.enableVerticalScrollbar
                === uiGridConstants.scrollbars.ALWAYS) {
             ...
             *
             */
            self.api.registerMethod('core', 'handleWindowResize',
             ...
             *
             */
            self.api.registerMethod('core', 'addRowHeaderColumn',
             ...
             *
             */
            self.api.registerMethod('core', 'sortHandleNulls',
                rowSorter.handleNulls);
             ...
             *
             */
            self.api.registerMethod('core', 'notifyDataChange',
                this.notifyDataChange);

            self.registerDataChangeCallback(self.columnRefreshCallback,
                [uiGridConstants.dataChange.COLUMN]);
            self.registerDataChangeCallback(self.processRowsCallback,
                [uiGridConstants.dataChange.EDIT]);
          ...
          Grid.prototype.getFooterStyles = function () {
            var style = '.grid' + this.id
                + ' .ui-grid-footer-aggregates-row { height: '
                + this.options.columnFooterHeight + 'px; }';
            style += ' .grid' + this.id + ' .ui-grid-footer-info { height: '
                + this.options.gridFooterHeight + 'px; }';
            return style;
          };
          ...
          Grid.prototype.buildColumnDefsFromData = function (dataRows) {
            this.options.columnDefs = gridUtil.getColumnsFromData(dataRows,
          ...
          Grid.prototype.assignTypes = function () {
            ...
            self.options.columnDefs.forEach(function (colDef, index) {
              ...
              if (!colDef.type) {
                ...
                if (firstRow) {
                  colDef.type = gridUtil.guessType(
                      self.getCellValue(firstRow, col));
                } else {
                  gridUtil.logWarn(
                      'Unable to assign type from data, so defaulting to string');
                  colDef.type = 'string';
                }
              }
            });
          ...
          Grid.prototype.addRowHeaderColumn = function addRowHeaderColumn(colDef) {
            ...
            var rowHeaderCol = new GridColumn(colDef,
                self.rowHeaderColumns.length, self);
          ...
          Grid.prototype.buildColumns = function buildColumns(opts) {
            ...
            self.options.columnDefs.forEach(function (colDef, index) {
              ...
              self.columnBuilders.forEach(function (builder) {
                builderPromises.push(
                    builder.call(self, colDef, col, self.options));
              });
            });
            ...
            if (!!options.orderByColumnDefs) {
              ...
              for (i = 0; i < self.options.columnDefs.length; i++) {
                ...
                if (self.columns[i + headerOffset].name
                    !== self.options.columnDefs[i].name) {
                  // Replace the one in the cache with the appropriate column
                  columnCache[i + headerOffset] = self.getColumn(
                      self.options.columnDefs[i].name);
                } else {
                  // Otherwise just copy over the one from the initial columns
                  columnCache[i + headerOffset] = self.columns[i
                  + headerOffset];
                }
              }
              ...
              Array.prototype.splice.apply(self.columns,
                  [0, 0].concat(columnCache));
            }
          ...
          Grid.prototype.preCompileCellTemplates = function () {
            ...
            this.columns.forEach(function (col) {
              var html = col.cellTemplate.replace(
                  uiGridConstants.MODEL_COL_FIELD,
                  self.getQualifiedColField(col));
              html = html.replace(uiGridConstants.COL_FIELD,
                  'grid.getCellValue(row, col)');
          ...
          Grid.prototype.createRightContainer = function () {
            if (!this.hasRightContainer()) {
              this.renderContainers.right = new GridRenderContainer('right',
                  this, {disableColumnOffset: true});
            }
          };
            ...
            if (colDef.name === undefined && colDef.field !== undefined) {
              ...
              if (foundName) {
                ...
                if (foundColumns.length === 0) {
                  ...
                  var lastNum = foundColumns[foundColumns.length
                  - 1].displayName.match(nameRE)[1];
          ...
          Grid.prototype.modifyRows = function modifyRows(newRawData) {
            var self = this,
                ...
                found,
                newRow;
            if ((self.options.useExternalSorting
                || self.getColumnSorting().length === 0) && newRawData.length
                > 0) {
              ...
              for (i = 0; i < newRawData.length; i++) {
                ...
                if (found) {
                  ...
                  newRow = self.processRowBuilders(
                      new GridRow(newRawRow, i, self));
                }
...
//        }
              ...
              if (self.rows.length === 0 && newRawData.length > 0) {
                ...
                newRows = newRows.concat(
                    self.newInN(self.rows, unfoundNew, 'entity'));

                self.addRows(newRows);

                var deletedRows = self.getDeletedRows(
                    (unfoundOldRows || self.rows), newRawData);
          ...
          Grid.prototype.addRows = function addRows(newRawData) {
            ...
            for (var i = 0; i < newRawData.length; i++) {
              var newRow = self.processRowBuilders(
                  new GridRow(newRawData[i], i + existingRowCount, self));
          ...
          Grid.prototype.registerRowsProcessor = function registerRowsProcessor(processor) {
            if (!angular.isFunction(processor)) {
              throw 'Attempt to register non-function rows processor: '
              + processor;
            }
          ...
          Grid.prototype.processRowsProcessors = function processRowsProcessors(renderableRows) {
            ...
            function startProcessor(i, renderedRowsToProcess) {
              ...
              return $q.when(
                  processor.call(self, renderedRowsToProcess, self.columns))
              .then(function handleProcessedRows(processedRows) {
                // Check for errors
                if (!processedRows) {
                  throw "Processor at index " + i
          ...
          Grid.prototype.setVisibleRows = function setVisibleRows(rows) {
            ...
            for (var ri = 0; ri < rows.length; ri++) {
              ...
              if (row.visible) {
                ...
                if (typeof (row.renderContainer) !== 'undefined'
                    && row.renderContainer) {
                  self.renderContainers[row.renderContainer].visibleRowCache.push(
                      row);
                }
          ...
          Grid.prototype.processColumnsProcessors = function processColumnsProcessors(renderableColumns) {
            ...
            function startProcessor(i, renderedColumnsToProcess) {
              ...
              //   (note: it's wrapped in $q.when() in case the processor does not return a promise)
              return $q.when(
                  processor.call(self, renderedColumnsToProcess, self.rows))
          ...
          Grid.prototype.setVisibleColumns = function setVisibleColumns(columns) {
            ...
            for (var ci = 0; ci < columns.length; ci++) {
              ...
              if (column.visible) {
                ...
                if (typeof (column.renderContainer) !== 'undefined'
                    && column.renderContainer) {
                  self.renderContainers[column.renderContainer].visibleColumnCache.push(
                      column);
                }
                // If not, put it into the body container
                else {
          ...
          Grid.prototype.getCellValue = function getCellValue(row, col) {
            ...
            if (!self.cellValueGetterCache[col.colDef.name]) {
              self.cellValueGetterCache[col.colDef.name] = $parse(
                  row.getEntityQualifiedColField(col));
            }
          ...
          Grid.prototype.getColumnSorting = function getColumnSorting() {
            ...
            myCols.sort(rowSorter.prioritySort).forEach(function (col) {
              if (col.sort && typeof (col.sort.direction) !== 'undefined'
                  && col.sort.direction && (col.sort.direction
                      === uiGridConstants.ASC || col.sort.direction
          ...
          Grid.prototype.sortColumn = function sortColumn(column,
              directionOrAdd, add) {
            var self = this,
                direction = null;
            ...
            if (!direction) {
              ...
              if (column.sort.direction && column.sort.direction
                  === uiGridConstants.ASC) {
                column.sort.direction = uiGridConstants.DESC;
              } else if (column.sort.direction && column.sort.direction
                  === uiGridConstants.DESC) {
          ...
          Grid.prototype.refresh = function refresh() {
            ...
            var p1 = self.processRowsProcessors(self.rows).then(
                function (renderableRows) {
                  self.setVisibleRows(renderableRows);
                });

            var p2 = self.processColumnsProcessors(self.columns).then(
                function (renderableColumns) {
          ...
          Grid.prototype.refreshCanvas = function (buildStyles) {
            ...
            for (var containerId in self.renderContainers) {
              if (self.renderContainers.hasOwnProperty(containerId)) {
                ...
                if (container.canvasWidth === null || isNaN(
              ...
              $timeout(function () {
                ...
                for (i = 0; i < containerHeadersToRecalc.length; i++) {
                  ...
                  if (container.header) {
                    ...
                    var headerHeight = gridUtil.outerElementHeight(
                        container.header);
                    ...
                    var topBorder = gridUtil.getBorderSize(container.header,
                        'top');
                    var bottomBorder = gridUtil.getBorderSize(container.header,
                        'bottom');
                    var innerHeaderHeight = parseInt(
                        headerHeight - topBorder - bottomBorder, 10);

                    innerHeaderHeight = innerHeaderHeight < 0 ? 0
                        : innerHeaderHeight;
...
(function () {
  ...
  angular.module('ui.grid')
  .factory('GridApi',
      ['$q', '$rootScope', 'gridUtil', 'uiGridConstants', 'GridRow',
        ...
        function ($q, $rootScope, gridUtil, uiGridConstants, GridRow,
          ...
          var GridApi = function GridApi(grid) {
             ...
             */
            this.registerMethod('core', 'setRowInvisible',
                GridRow.prototype.setRowInvisible);
             ...
             * @param {object} rowEntity gridOptions.data[] array instance
             */
            this.registerMethod('core', 'clearRowInvisible',
                GridRow.prototype.clearRowInvisible);
             ...
             */
            this.registerMethod('core', 'getVisibleRows',
           ...
           * </pre>
           */
          GridApi.prototype.suppressEvents = function (listenerFuncs,
              callBackFn) {
            var self = this;
            var listeners = angular.isArray(listenerFuncs) ? listenerFuncs
                : [listenerFuncs];
            ...
            foundListeners.forEach(function (l) {
              l.dereg = registerEventWithAngular(l.eventId, l.handler,
                  self.grid, l._this);
            });
          ...
          GridApi.prototype.registerEvent = function (featureName, eventName) {
            ...
            feature.raise[eventName] = function () {
              $rootScope.$emit.apply($rootScope,
                  [eventId].concat(Array.prototype.slice.call(arguments)));
            };

            // gridUtil.logDebug('Creating on event method ' + featureName + '.on.' + eventName);
            feature.on[eventName] = function (scope, handler, _this) {
              var deregAngularOn = registerEventWithAngular(eventId, handler,
                  self.grid, _this);
           ...
           */
          GridApi.prototype.registerMethod = function (featureName, methodName,
              callBackFn, _this) {
            if (!this[featureName]) {
              this[featureName] = {};
            }
            ...
            feature[methodName] = gridUtil.createBoundedWrapper(
                _this || this.grid, callBackFn);
          };
...
(function () {
  ...
  angular.module('ui.grid')
  .factory('GridColumn', ['gridUtil', 'uiGridConstants', 'i18nService',
    function (gridUtil, uiGridConstants, i18nService) {
       ...
       */
      GridColumn.prototype.setPropertyOrDefault = function (colDef, propName,
          defaultValue) {
        var self = this;
      ...
      GridColumn.prototype.updateColumnDef = function (colDef, isNew) {
        ...
        if (colDef.name === undefined) {
          throw new Error('colDef.name is required for column at index '
              + self.grid.options.columnDefs.indexOf(colDef));
        }

        self.displayName = (colDef.displayName === undefined)
            ? gridUtil.readableColumnName(colDef.name) : colDef.displayName;

        var parseErrorMsg = "Cannot parse column width '" + colDef.width
            + "' for column named '" + colDef.name + "'";

        // If width is not defined, set it to a single star
        if (gridUtil.isNullOrUndefined(self.width) || !angular.isNumber(
            self.width)) {
        ...
        self.aggregationType = angular.isDefined(colDef.aggregationType)
            ? colDef.aggregationType : null;
        self.footerCellTemplate = angular.isDefined(colDef.footerCellTemplate)
            ? colDef.footerCellTemplate : null;
         ...
         *
         */
        self.headerCellFilter = colDef.headerCellFilter
            ? colDef.headerCellFilter : "";
         ...
         *
         */
        self.footerCellFilter = colDef.footerCellFilter
            ? colDef.footerCellFilter : "";

        self.visible = gridUtil.isNullOrUndefined(colDef.visible)
            || colDef.visible;
        ...
        self.enableSorting = typeof (colDef.enableSorting) !== 'undefined'
        ...
        self.enableFiltering = typeof (colDef.enableFiltering) !== 'undefined'
    ...
    */
        ...
        GridColumn.prototype.unsort = function () {
          this.sort = {};
          self.grid.api.core.raise.sortChanged(self,
              self.grid.getColumnSorting());
        };
      ...
      GridColumn.prototype.getColClassDefinition = function () {
        return ' .grid' + this.grid.id + ' ' + this.getColClass(true)
            + ' { width: ' + this.drawnWidth + 'px; }';
      };
      ...
      GridColumn.prototype.getAggregationValue = function () {
        ...
        if (angular.isFunction(self.aggregationType)) {
          ...
          return result;
            ...
            === uiGridConstants.aggregationTypes.min) {
...
(function () {
  ...
  angular.module('ui.grid')
  .factory('GridOptions',
      ['gridUtil', 'uiGridConstants', function (gridUtil, uiGridConstants) {
        ...
        return {
          initialize: function (baseOptions) {
             ...
             *
             */
            baseOptions.onRegisterApi = baseOptions.onRegisterApi
                || angular.noop();
            ...
            baseOptions.excludeProperties = baseOptions.excludeProperties
                || ['$$hashKey'];
             ...
             */
            baseOptions.enableRowHashing = baseOptions.enableRowHashing
                !== false;
             ...
             */
            baseOptions.rowIdentity = baseOptions.rowIdentity
             ...
             */
            baseOptions.getRowIdentity = baseOptions.getRowIdentity
             ...
             */
            baseOptions.showHeader = typeof (baseOptions.showHeader)
            !== "undefined" ? baseOptions.showHeader : true;
            ...
            if (!baseOptions.showHeader) {
              ...
              baseOptions.headerRowHeight = typeof (baseOptions.headerRowHeight)
              !== "undefined" ? baseOptions.headerRowHeight : 30;
            }
             ...
             */
            baseOptions.minRowsToShow = typeof (baseOptions.minRowsToShow)
            !== "undefined" ? baseOptions.minRowsToShow : 10;
             ...
             */
            baseOptions.showColumnFooter = baseOptions.showColumnFooter
                === true;
             ...
             *
             */
            baseOptions.columnFooterHeight = typeof (baseOptions.columnFooterHeight)
            !== "undefined" ? baseOptions.columnFooterHeight : 30;
            baseOptions.gridFooterHeight = typeof (baseOptions.gridFooterHeight)
            !== "undefined" ? baseOptions.gridFooterHeight : 30;

            baseOptions.columnWidth = typeof (baseOptions.columnWidth)
            !== "undefined" ? baseOptions.columnWidth : 50;
             ...
             *
             */
            baseOptions.maxVisibleColumnCount = typeof (baseOptions.maxVisibleColumnCount)
            !== "undefined" ? baseOptions.maxVisibleColumnCount : 200;
             ...
             */
            baseOptions.virtualizationThreshold = typeof (baseOptions.virtualizationThreshold)
            !== "undefined" ? baseOptions.virtualizationThreshold : 20;
             ...
             */
            baseOptions.columnVirtualizationThreshold = typeof (baseOptions.columnVirtualizationThreshold)
            !== "undefined" ? baseOptions.columnVirtualizationThreshold : 10;
             ...
             * Defaults to 4
             */
            baseOptions.excessRows = typeof (baseOptions.excessRows)
            !== "undefined" ? baseOptions.excessRows : 4;
            /**
             * @ngdoc property
             ...
             */
            baseOptions.scrollThreshold = typeof (baseOptions.scrollThreshold)
            !== "undefined" ? baseOptions.scrollThreshold : 4;
             ...
             * Defaults to 4
             */
            baseOptions.excessColumns = typeof (baseOptions.excessColumns)
            !== "undefined" ? baseOptions.excessColumns : 4;
            /**
             * @ngdoc property
             ...
             * @description Defaults to 4
             */
            baseOptions.horizontalScrollThreshold = typeof (baseOptions.horizontalScrollThreshold)
            !== "undefined" ? baseOptions.horizontalScrollThreshold : 2;
             ...
             */
            baseOptions.scrollThrottle = typeof (baseOptions.scrollThrottle)
            !== "undefined" ? baseOptions.scrollThrottle : 70;
             ...
             */
            baseOptions.enableVerticalScrollbar = typeof (baseOptions.enableVerticalScrollbar)
            !== "undefined" ? baseOptions.enableVerticalScrollbar
                : uiGridConstants.scrollbars.ALWAYS;
             ...
             * Supported values: uiGridConstants.scrollbars.ALWAYS, uiGridConstants.scrollbars.NEVER
             */
            baseOptions.enableHorizontalScrollbar = typeof (baseOptions.enableHorizontalScrollbar)
            !== "undefined" ? baseOptions.enableHorizontalScrollbar
                : uiGridConstants.scrollbars.ALWAYS;
             ...
             */
            baseOptions.minimumColumnSize = typeof (baseOptions.minimumColumnSize)
            !== "undefined" ? baseOptions.minimumColumnSize : 10;
             ...
             */
            baseOptions.rowEquality = baseOptions.rowEquality
             ...
             */
            baseOptions.rowTemplate = baseOptions.rowTemplate
                || 'ui-grid/ui-grid-row';
  ...
  .factory('GridRenderContainer',
      ['gridUtil', 'uiGridConstants', function (gridUtil, uiGridConstants) {
        ...
        GridRenderContainer.prototype.minRowsToRender = function minRowsToRender() {
          ...
          for (var i = self.visibleRowCache.length - 1;
            ...
            if (totalWidth < viewportWidth) {
              ...
              for (var j = i; j >= i - min; j--) {
                currWidth += self.visibleColumnCache[j].drawnWidth
                    ? self.visibleColumnCache[j].drawnWidth : 0;
              }
        ...
        GridRenderContainer.prototype.getViewportHeight = function getViewportHeight() {
          ...
          var headerHeight = (self.headerHeight) ? self.headerHeight
              : self.grid.headerHeight;

          var viewPortHeight = self.grid.gridHeight - headerHeight
              - self.grid.footerHeight;
        ...
        GridRenderContainer.prototype.getCanvasHeight = function getCanvasHeight() {
          ...
          self.grid.api.core.raise.canvasHeightChanged(oldCanvasHeight,
              self.$$canvasHeight);
        ...
        GridRenderContainer.prototype.adjustScrollVertical = function adjustScrollVertical(scrollTop,
            scrollPercentage, force) {
          if (this.prevScrollTop === scrollTop && !force) {
            return;
          }

          if (typeof (scrollTop) === 'undefined' || scrollTop === undefined
              || scrollTop === null) {
            scrollTop = (this.getCanvasHeight() - this.getCanvasWidth())
                * scrollPercentage;
          }
        ...
        GridRenderContainer.prototype.adjustScrollHorizontal = function adjustScrollHorizontal(scrollLeft,
            scrollPercentage, force) {
          if (this.prevScrollLeft === scrollLeft && !force) {
            return;
          }

          if (typeof (scrollLeft) === 'undefined' || scrollLeft === undefined
              || scrollLeft === null) {
            scrollLeft = (this.getCanvasWidth() - this.getViewportWidth())
                * scrollPercentage;
          }
        ...
        GridRenderContainer.prototype.adjustRows = function adjustRows(scrollTop,
            scrollPercentage, postDataLoaded) {
          var self = this;
          ...
          if ((typeof (scrollPercentage) === 'undefined' || scrollPercentage
              === null) && scrollTop) {
            scrollPercentage = scrollTop / self.getVerticalScrollLength();
          }

          var rowIndex = Math.ceil(
              Math.min(maxRowIndex, maxRowIndex * scrollPercentage));
          ...
          if (rowCache.length > self.grid.options.virtualizationThreshold) {
            if (!(typeof (scrollTop) === 'undefined' || scrollTop === null)) {
              ...
              if (!self.grid.options.enableInfiniteScroll && self.prevScrollTop
                  < scrollTop && rowIndex < self.prevRowScrollIndex
                  + self.grid.options.scrollThreshold && rowIndex
                  < maxRowIndex) {
                return;
              }
            ...
            if (self.grid.options.enableInfiniteScroll
                && self.grid.scrollDirection
                !== uiGridConstants.scrollDirection.NONE && postDataLoaded) {
              var findIndex = null;
              var i = null;
              if (self.grid.scrollDirection
                  === uiGridConstants.scrollDirection.UP) {
                ...
                for (i = 0; i < rowCache.length; i++) {
                  if (rowCache[i].entity.$$hashKey
                      === self.renderedRows[findIndex].entity.$$hashKey) {
                    rowIndex = i;
                    break;
                  }
                }
                rangeStart = Math.max(0, rowIndex);
                rangeEnd = Math.min(rowCache.length,
                    rangeStart + self.grid.options.excessRows + minRows);
              } else if (self.grid.scrollDirection
                  === uiGridConstants.scrollDirection.DOWN) {
                findIndex = minRows;
                for (i = 0; i < rowCache.length; i++) {
                  if (rowCache[i].entity.$$hashKey
                      === self.renderedRows[findIndex].entity.$$hashKey) {
                ...
                rangeStart = Math.max(0,
                    rowIndex - self.grid.options.excessRows - minRows);
                rangeEnd = Math.min(rowCache.length,
                    rowIndex + minRows + self.grid.options.excessRows);
              }
            } else {
            ...
            newRange = [0,
              Math.max(maxLen, minRows + self.grid.options.excessRows)];
          }
        ...
        GridRenderContainer.prototype.adjustColumns = function adjustColumns(scrollLeft,
          ...
          var colIndex = Math.ceil(
              Math.min(maxColumnIndex, maxColumnIndex * scrollPercentage));
          ...
          if (columnCache.length
              > self.grid.options.columnVirtualizationThreshold
              && self.getCanvasWidth() > self.getViewportWidth()) {
      ...
      if (self.prevScrollLeft > scrollLeft && colIndex > self.prevColumnScrollIndex - self.grid.options.horizontalScrollThreshold && colIndex < maxColumnIndex) {
        return;
            ...
            var rangeStart = Math.max(0,
                colIndex - self.grid.options.excessColumns);
            var rangeEnd = Math.min(columnCache.length,
                colIndex + minCols + self.grid.options.excessColumns);
            ...
            newRange = [0,
              Math.max(maxLen, minCols + self.grid.options.excessColumns)];
          }
        ...
        GridRenderContainer.prototype.updateViewableRowRange = function updateViewableRowRange(renderedRange) {
          ...
          var rowArr = this.visibleRowCache.slice(renderedRange[0],
              renderedRange[1]);
        ...
        GridRenderContainer.prototype.updateViewableColumnRange = function updateViewableColumnRange(renderedRange) {
          ...
          var columnArr = this.visibleColumnCache.slice(renderedRange[0],
              renderedRange[1]);
        ...
        GridRenderContainer.prototype.updateColumnWidths = function () {
          ...
          columnCache.forEach(function (column, i) {
            ...
            if (angular.isString(column.width) && column.width.indexOf('*')
                !== -1) { //  we need to save it until the end to do the calulations on the remaining width.
              ...
              canvasWidth = parseInt(canvasWidth, 10) + parseInt(column.width,
                  10);
          ...
          if (percentArray.length > 0) {
            ...
            for (i = 0; i < percentArray.length; i++) {
              ...
              } else if (column.colDef.maxWidth && colWidth
                  > column.colDef.maxWidth) {
          ...
          if (leftoverWidth > 0 && canvasWidth > 0 && canvasWidth
              < availableWidth) {
        ...
        GridRenderContainer.prototype.getViewPortStyle = function () {
          ...
          if (self.name === 'body') {
            styles['overflow-x'] = self.grid.options.enableHorizontalScrollbar
            === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'scroll';
            if (!self.grid.isRTL()) {
              if (self.grid.hasRightContainerColumns()) {
                styles['overflow-y'] = 'hidden';
              } else {
                styles['overflow-y'] = self.grid.options.enableVerticalScrollbar
                === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'scroll';
              }
            } else {
            ...
            styles['overflow-y'] = self.grid.isRTL()
                ? (self.grid.options.enableVerticalScrollbar
                === uiGridConstants.scrollbars.NEVER ? 'hidden' : 'scroll')
                : 'hidden';
          } else {
            styles['overflow-x'] = 'hidden';
  ...
  .factory('ScrollEvent', ['gridUtil', function (gridUtil) {
    ...
    ScrollEvent.prototype.getNewScrollLeft = function (colContainer, viewport) {
      ...
      if (!self.newScrollLeft) {
        var scrollWidth = (colContainer.getCanvasWidth()
            - colContainer.getViewportWidth());

        var oldScrollLeft = gridUtil.normalizeScrollLeft(viewport);

        var scrollXPercentage;
        if (typeof (self.x.percentage) !== 'undefined' && self.x.percentage
            !== undefined) {
          scrollXPercentage = self.x.percentage;
        } else if (typeof (self.x.pixels) !== 'undefined' && self.x.pixels
            !== undefined) {
          scrollXPercentage = self.x.percentage = (oldScrollLeft
              + self.x.pixels) / scrollWidth;
        } else {
          throw new Error(
    ...
    ScrollEvent.prototype.getNewScrollTop = function (rowContainer, viewport) {
      ...
      if (!self.newScrollTop) {
        ...
        if (typeof (self.y.percentage) !== 'undefined' && self.y.percentage
            !== undefined) {
          scrollYPercentage = self.y.percentage;
        } else if (typeof (self.y.pixels) !== 'undefined' && self.y.pixels
            !== undefined) {
          scrollYPercentage = self.y.percentage = (oldScrollTop + self.y.pixels)
              / scrollLength;
        } else {
          throw new Error(
...
(function () {
  ...
  angular.module('ui.grid').service('gridClassFactory',
      ['gridUtil', '$q', '$compile', '$templateCache', 'uiGridConstants',
        'Grid', 'GridColumn', 'GridRow',
        function (gridUtil, $q, $compile, $templateCache, uiGridConstants, Grid,
            GridColumn, GridRow) {
          ...
          var service = {
              ...
              if (grid.options.rowTemplate) {
                ...
                .then(
                    ...
                    function (res) {
                      ...
                      throw new Error("Couldn't fetch/use row template '"
                          + grid.options.rowTemplate + "'");
                    });
              }
            ...
            defaultColumnBuilder: function (colDef, col, gridOptions) {
              ...
              col.cellTemplatePromise = gridUtil.getTemplate(
                  col.providedCellTemplate);
              templateGetPromises.push(col.cellTemplatePromise
                  .then(
                      function (template) {
                        col.cellTemplate = template.replace(
                            uiGridConstants.CUSTOM_FILTERS,
                            col.cellFilter ? "|" + col.cellFilter : "");
                      },
                      function (res) {
                        throw new Error("Couldn't fetch/use colDef.cellTemplate '"
                            + colDef.cellTemplate + "'");
                      })
              );

              templateGetPromises.push(
                  gridUtil.getTemplate(col.providedHeaderCellTemplate)
                  .then(
                      function (template) {
                        col.headerCellTemplate = template.replace(
                            uiGridConstants.CUSTOM_FILTERS,
                            col.headerCellFilter ? "|" + col.headerCellFilter
                                : "");
                      },
                      function (res) {
                        throw new Error(
                            "Couldn't fetch/use colDef.headerCellTemplate '"
                            + colDef.headerCellTemplate + "'");
                      })
              );

              templateGetPromises.push(
                  gridUtil.getTemplate(col.providedFooterCellTemplate)
                  .then(
                      function (template) {
                        col.footerCellTemplate = template.replace(
                            uiGridConstants.CUSTOM_FILTERS,
                            col.footerCellFilter ? "|" + col.footerCellFilter
                                : "");
                      },
                      function (res) {
                        throw new Error(
                            "Couldn't fetch/use colDef.footerCellTemplate '"
              ...
              // Row has its own template assigned
                ...
                .then(function (template) {
                    ...
                    function (res) {
                      // Todo handle response error here?
                      throw new Error(
                          "Couldn't fetch/use row template '" + row.rowTemplate
                          + "'");
                    });
              }
...
(function () {
  ...
  module.service('rowSearcher',
      ['gridUtil', 'uiGridConstants', function (gridUtil, uiGridConstants) {
        ...
        rowSearcher.setupFilters = function setupFilters(filters) {
          ...
          for (var i = 0; i < filtersLength; i++) {
            ...
            if (filter.noTerm || filter.term) {
              ...
              newFilter.flags = angular.extend({caseSensitive: false},
                  filter.flags);

              if (newFilter.condition === uiGridConstants.filter.STARTS_WITH) {
                newFilter.startswithRE = new RegExp('^' + newFilter.term,
                    regexpFlags);
              }

              if (newFilter.condition === uiGridConstants.filter.ENDS_WITH) {
                newFilter.endswithRE = new RegExp(newFilter.term + '$',
                    regexpFlags);
              }
              ...
              if (newFilter.condition === uiGridConstants.filter.EXACT) {
                newFilter.exactRE = new RegExp('^' + newFilter.term + '$',
                    regexpFlags);
              }
         ...
         */
        rowSearcher.runColumnFilter = function runColumnFilter(grid, row,
            column, filter) {
         ...
         */
        rowSearcher.searchColumn = function searchColumn(grid, row, column,
            filters) {
     ...
     */
          ...
          for (var i = 0; i < colsLength; i++) {
            ...
            if (typeof (col.filters) !== 'undefined' && (col.filters.length > 1
                || col.filters.length === 1 && (typeof (col.filters[0].term)
                    !== 'undefined' && col.filters[0].term
                    || col.filters[0].noTerm))) {
              filterData.push(
                  {col: col, filters: rowSearcher.setupFilters(col.filters)});
            } else if (typeof (col.filter) !== 'undefined' && col.filter
                && (typeof (col.filter.term) !== 'undefined' && col.filter.term
                    || col.filter.noTerm)) {
              filterData.push(
                  {col: col, filters: rowSearcher.setupFilters([col.filter])});
            }
          }
          ...
          if (filterData.length > 0) {
            ...
            var foreachRow = function (grid, row, col, filters) {
              if (row.forceInvisible || !rowSearcher.searchColumn(grid, row,
                  col, filters)) {
                row.visible = false;
              }
            };
...
(function () {
  ...
  module.service('rowSorter',
      ['$parse', 'uiGridConstants', function ($parse, uiGridConstants) {
        ...
        // /^[-+]?[£$¤¥]?[\d,.]+%?$/
        var numberStrRegex = new RegExp(
            '^[-+]?' + currencyRegexStr + '[\\d,.]+' + currencyRegexStr
            + '%?$');
        ...
        rowSorter.sort = function rowSorterSort(grid, rows, columns) {
          ...
          columns.forEach(function (col) {
            if (col.sort && col.sort.direction && (col.sort.direction
                === uiGridConstants.ASC || col.sort.direction
                === uiGridConstants.DESC)) {
              sortCols.push(col);
            }
...
(function () {
   ...
   */
  module.service('gridUtil',
      ['$log', '$window', '$document', '$http', '$templateCache', '$timeout',
        '$injector', '$q', '$interpolate', 'uiGridConstants',
        function ($log, $window, $document, $http, $templateCache, $timeout,
            $injector, $q, $interpolate, uiGridConstants) {
          var s = {
            ...
            readableColumnName: function (columnName) {
              ...
              .replace(/^[A-Z]+$/, function (match) {
                return angular.lowercase(
                    angular.uppercase(match.charAt(0)) + match.slice(1));
              })
            ...
            getColumnsFromData: function (data, excludeProperties) {
              ...
              if (!data || typeof (data[0]) === 'undefined' || data[0]
                  === undefined) {
                return [];
              }
          ...
          alert(contents);
            ...
            getTemplate: function (template) {
              ...
              .then(
                  ...
                  function (err) {
                    throw new Error(
                        "Could not get template " + template + ": " + err);
                  }
              )
            ...
            arrayContainsObjectWithProperty: function (array, propertyName,
                propertyValue) {
              var found = false;
            ...
            hashKey: function hashKey(obj) {
              ...
              if (objType === 'object' && obj !== null) {
                if (typeof (key = obj.$$hashKey) === 'function') {
                  ...
                  key = obj.$$hashKey();
                } else if (typeof (obj.$$hashKey) !== 'undefined'
                    && obj.$$hashKey) {
                  key = obj.$$hashKey;
          ...
          ['width', 'height'].forEach(function (name) {
            ...
            s['outerElement' + capsName] = function (elem, margin) {
              return elem ? s['element' + capsName].call(this, elem,
          ...
          s.getBorderSize = function getBorderSize(elem, borderType) {
            ...
            if (borderType) {
              borderType = 'border' + borderType.charAt(0).toUpperCase()
                  + borderType.slice(1);
            } else {
              borderType = 'border';
            }
           ...
           */
          s.denormalizeScrollLeft = function denormalizeScrollLeft(element,
              scrollLeft) {
            if (typeof (element.length) !== 'undefined' && element.length) {
              element = element[0];
            }
          ...
          s.preEval = function (path) {
            ...
            if (m) {
              return (m[1] ? s.preEval(m[1]) : m[1]) + m[2] + (m[3] ? s.preEval(
                  m[3]) : m[3]);
            } else {
              ...
              angular.forEach(parts, function (part) {
                preparsed.push(
                    part.replace(uiGridConstants.FUNC_REGEXP, '\']$1'));
              });
...
//        $provide.decorator('i18nService', ['$delegate', function($delegate) {}])}]);
            ...
            if (lang) {
              ...
              $attrs.$observe(alias, function () {
                i18nService.setCurrentLang(
                    $attrs[alias] || i18nConstants.DEFAULT_LANG);
              });
            }
          }
        };
          ...
          pre: function ($scope, $elm, $attrs) {
            ...
            if ($attrs.$$observers) {
              ...
              observer = $attrs.$observe(prop, function (result) {
                if (result) {
                  $elm.html(
                      $parse(result)(i18nService.getCurrentLang()) || missing);
                }
              });
            }
            var getter = $parse(token);
            var listener = $scope.$on(i18nConstants.UPDATE_EVENT,
                function (evt) {
                  if (observer) {
  ...
  angular.forEach(DIRECTIVE_ALIASES, function (alias) {
    module.directive(alias,
        ['$parse', 'i18nService', 'i18nConstants', uitDirective]);
  });
...
(function () {
  ...
  module.directive('uiGridAutoResize',
      ['$timeout', 'gridUtil', function ($timeout, gridUtil) {
        return {
          require: 'uiGrid',
          scope: false,
          link: function ($scope, $elm, $attrs, uiGridCtrl) {
            ...
            function startTimeout() {
              ...
              resizeTimeoutId = setTimeout(function () {
                ...
                if (newGridHeight !== prevGridHeight || newGridWidth
                    !== prevGridWidth) {
...
(function () {
  ...
  module.factory('uiGridCellNavFactory',
      ['gridUtil', 'uiGridConstants', 'uiGridCellNavConstants', '$q',
        function (gridUtil, uiGridConstants, uiGridCellNavConstants, $q) {
           ...
           */
          var UiGridCellNav = function UiGridCellNav(rowContainer, colContainer,
              leftColContainer, rightColContainer) {
            this.rows = rowContainer.visibleRowCache;
            this.columns = colContainer.visibleColumnCache;
            this.leftColumns = leftColContainer
                ? leftColContainer.visibleColumnCache : [];
            this.rightColumns = rightColContainer
                ? rightColContainer.visibleColumnCache : [];
          ...
          UiGridCellNav.prototype.getFocusableCols = function () {
            var allColumns = this.leftColumns.concat(this.columns,
                this.rightColumns);
          ...
          UiGridCellNav.prototype.getNextRowCol = function (direction, curRow,
              curCol) {
            switch (direction) {
          ...
          UiGridCellNav.prototype.getRowColLeft = function (curRow, curCol) {
            ...
            var nextColIndex = curColIndex === 0 ? focusableCols.length - 1
                : curColIndex - 1;
            ...
            if (nextColIndex > curColIndex) {
              if (curRowIndex === 0) {
                ...
                return new RowCol(focusableRows[curRowIndex - 1],
                    focusableCols[nextColIndex]);
              }
            } else {
          ...
          UiGridCellNav.prototype.getRowColRight = function (curRow, curCol) {
            ...
            var nextColIndex = curColIndex === focusableCols.length - 1 ? 0
                : curColIndex + 1;
          ...
          UiGridCellNav.prototype.getRowColDown = function (curRow, curCol) {
            ...
            if (curRowIndex === focusableRows.length - 1) {
              ...
              //down one row
              return new RowCol(focusableRows[curRowIndex + 1],
                  focusableCols[curColIndex]);
            }
          };

          UiGridCellNav.prototype.getRowColPageDown = function (curRow,
              curCol) {
            var focusableCols = this.getFocusableCols();
            var focusableRows = this.getFocusableRows();
            var curColIndex = focusableCols.indexOf(curCol);
            ...
            if (curRowIndex >= focusableRows.length - pageSize) {
              return new RowCol(focusableRows[focusableRows.length - 1],
                  focusableCols[curColIndex]); //return last row
            } else {
              //down one page
              return new RowCol(focusableRows[curRowIndex + pageSize],
                  focusableCols[curColIndex]);
            }
          };
   ...
   */
  module.service('uiGridCellNavService',
      ['gridUtil', 'uiGridConstants', 'uiGridCellNavConstants', '$q',
        'uiGridCellNavFactory', 'ScrollEvent',
        function (gridUtil, uiGridConstants, uiGridCellNavConstants, $q,
            UiGridCellNav, ScrollEvent) {
          ...
          var service = {
                ...
                methods: {
                  cellNav: {
                    ...
                    rowColSelectIndex: function (rowCol) {
                      ...
                      var index = -1;
                      for (var i = 0; i < grid.cellNav.focusedCells.length;
                          i++) {
                        if (grid.cellNav.focusedCells[i].col.uid
                            === rowCol.col.uid &&
                            grid.cellNav.focusedCells[i].row.uid
                            === rowCol.row.uid) {
                          index = i;
                          break;
                        }
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               */
              gridOptions.modifierKeysToMultiSelectCells = gridOptions.modifierKeysToMultiSelectCells
                  === true;
            ...
            decorateRenderContainers: function (grid) {
              ...
              var rightContainer = grid.hasRightContainer()
                  ? grid.renderContainers.right : null;
              var leftContainer = grid.hasLeftContainer()
                  ? grid.renderContainers.left : null;

              if (leftContainer !== null) {
                grid.renderContainers.left.cellNav = new UiGridCellNav(
                    grid.renderContainers.body, leftContainer, rightContainer,
                    grid.renderContainers.body);
              }
              if (rightContainer !== null) {
                grid.renderContainers.right.cellNav = new UiGridCellNav(
                    grid.renderContainers.body, rightContainer,
                    grid.renderContainers.body, leftContainer);
              }

              grid.renderContainers.body.cellNav = new UiGridCellNav(
                  grid.renderContainers.body, grid.renderContainers.body,
            ...
            cellNavColumnBuilder: function (colDef, col, gridOptions) {
               ...
               *  <br/>Defaults to true
               */
              colDef.allowCellFocus = colDef.allowCellFocus === undefined ? true
                  : colDef.allowCellFocus;
            ...
            scrollTo: function (grid, rowEntity, colDef) {
              ...
              if (colDef !== null && typeof (colDef) !== 'undefined') {
                gridCol = grid.getColumn(
                    colDef.name ? colDef.name : colDef.field);
              }
            ...
            scrollToInternal: function (grid, gridRow, gridCol) {
              var scrollEvent = new ScrollEvent(grid, null, null,
                  'uiGridCellNavService.scrollToInternal');

              if (gridRow !== null) {
                var seekRowIndex = grid.renderContainers.body.visibleRowCache.indexOf(
                    gridRow);
                var totalRows = grid.renderContainers.body.visibleRowCache.length;
                var percentage = (seekRowIndex + (seekRowIndex / (totalRows
                    - 1))) / totalRows;
                scrollEvent.y = {percentage: percentage};
              }
              ...
              if (gridCol !== null) {
                scrollEvent.x = {
                  percentage: this.getLeftWidth(grid, gridCol)
                      / this.getLeftWidth(grid,
                          grid.renderContainers.body.visibleColumnCache[grid.renderContainers.body.visibleColumnCache.length
                          - 1])
                };
              }
            ...
            scrollToIfNecessary: function (grid, gridRow, gridCol) {
              var scrollEvent = new ScrollEvent(grid,
                  'uiGridCellNavService.scrollToIfNecessary');
              ...
              var topBound = grid.renderContainers.body.prevScrollTop
                  + grid.headerHeight;
              ...
              var bottomBound = grid.renderContainers.body.prevScrollTop
                  + grid.gridHeight - grid.headerHeight;
              ...
              var rightBound = grid.renderContainers.body.prevScrollLeft
                  + Math.ceil(grid.gridWidth);
              ...
              if (gridRow !== null) {
                ...
                var scrollLength = (grid.renderContainers.body.getCanvasHeight()
                    - grid.renderContainers.body.getViewportHeight());
                ...
                var pixelsToSeeRow = ((seekRowIndex + 1)
                    * grid.options.rowHeight);
                ...
                if (pixelsToSeeRow < topBound) {
                  ...
                  scrollPixels = grid.renderContainers.body.prevScrollTop
                      - (topBound - pixelsToSeeRow);
                ...
                // Otherwise if the scroll position we need to see the row is MORE than the bottom boundary, i.e. obscured below the bottom of the grid...
                  ...
                  //   to get the full position we need
                  scrollPixels = pixelsToSeeRow - bottomBound
              ...
              if (gridCol !== null) {
                ...
                // Total vertical scroll length of the grid
                var horizScrollLength = (grid.renderContainers.body.getCanvasWidth()
                    - grid.renderContainers.body.getViewportWidth());
                ...
                if (columnLeftEdge < leftBound) {
                  // Get the different between the top boundary and the required scroll position and subtract it from the current scroll position\
                  //   to get the full position we need
                  horizScrollPixels = grid.renderContainers.body.prevScrollLeft
                      - (leftBound - columnLeftEdge);
                ...
                // Otherwise if the scroll position we need to see the row is MORE than the bottom boundary, i.e. obscured below the bottom of the grid...
                  ...
                  // Get the different between the bottom boundary and the required scroll position and add it to the current scroll position
                  //   to get the full position we need
                  horizScrollPixels = columnRightEdge - rightBound
            ...
            getLeftWidth: function (grid, upToCol) {
              ...
              var lastIndex = grid.renderContainers.body.visibleColumnCache.indexOf(
                  upToCol);

              // total column widths up-to but not including the passed in column
              grid.renderContainers.body.visibleColumnCache.forEach(
                  function (col, index) {
                    if (index < lastIndex) {
              ...
              var percentage = lastIndex === 0 ? 0 : (lastIndex + 1)
                  / grid.renderContainers.body.visibleColumnCache.length;
  ...
  module.directive('uiGridCellnav',
      ['gridUtil', 'uiGridCellNavService', 'uiGridCellNavConstants',
              ...
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  uiGridCtrl.cellNav.broadcastFocus = function (rowCol,
                    ...
                    var rowColSelectIndex = uiGridCtrl.grid.api.cellNav.rowColSelectIndex(
                        rowCol);

                    if (grid.cellNav.lastRowCol === null || rowColSelectIndex
                        === -1) {
                      var newRowCol = new RowCol(row, col);
                      grid.api.cellNav.raise.navigate(newRowCol,
                          grid.cellNav.lastRowCol);
                    ...
                    } else if (grid.options.modifierKeysToMultiSelectCells
                        && modifierDown &&
                        rowColSelectIndex >= 0) {
                  ...
                  uiGridCtrl.cellNav.handleKeyDown = function (evt) {
                    ...
                    if (lastRowCol) {
                      ...
                      var rowCol = uiGridCtrl.grid.renderContainers[containerId].cellNav.getNextRowCol(
                          direction, lastRowCol.row, lastRowCol.col);
                      ...
                      uiGridCellNavService.scrollToIfNecessary(grid, rowCol.row,
                          rowCol.col);
  ...
  module.directive('uiGridRenderContainer',
      ['$timeout', '$document', 'gridUtil', 'uiGridConstants',
        'uiGridCellNavService', 'uiGridCellNavConstants',
        function ($timeout, $document, gridUtil, uiGridConstants,
            uiGridCellNavService, uiGridCellNavConstants) {
          return {
            replace: true,
  ...
  module.directive('uiGridCell',
      ['$timeout', '$document', 'uiGridCellNavService', 'gridUtil',
        'uiGridCellNavConstants', 'uiGridConstants',
        function ($timeout, $document, uiGridCellNavService, gridUtil,
            uiGridCellNavConstants, uiGridConstants) {
          return {
            ...
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
              ...
              $elm.find('div').on('click', function (evt) {
                uiGridCtrl.cellNav.broadcastCellNav(
                    new RowCol($scope.row, $scope.col),
              ...
              $scope.$on(uiGridCellNavConstants.CELL_NAV_EVENT,
                  function (evt, rowCol, modifierDown) {
                    if (rowCol.row === $scope.row &&
                        rowCol.col === $scope.col) {
                      if (uiGridCtrl.grid.options.modifierKeysToMultiSelectCells
                          && modifierDown &&
                          uiGridCtrl.grid.api.cellNav.rowColSelectIndex(rowCol)
                          === -1) {
                        clearFocus();
                      } else {
                        setFocused();
                      ...
                      if (rowCol.hasOwnProperty('eventType') && rowCol.eventType
                          === uiGridCellNavConstants.EVENT_TYPE.KEYDOWN) {
                        $elm.find('div')[0].focus();
                      }
                    } else if (!(uiGridCtrl.grid.options.modifierKeysToMultiSelectCells
                        && modifierDown)) {
                      clearFocus();
                    }
                  });
...
(function () {
   ...
   */
  module.service('uiGridEditService',
      ['$q', '$templateCache', 'uiGridConstants', 'gridUtil',
        function ($q, $templateCache, uiGridConstants, gridUtil) {
          ...
          var service = {
              ...
              var publicApi = {
                events: {
                  edit: {
                     ...
                     */
                    afterCellEdit: function (rowEntity, colDef, newValue,
                        oldValue) {
                    },
                    /**
                     * @ngdoc event
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               *  </pre>
               */
              gridOptions.cellEditableCondition = gridOptions.cellEditableCondition
              === undefined ? true : gridOptions.cellEditableCondition;
              ...
              gridOptions.enableCellEditOnFocus = gridOptions.enableCellEditOnFocus
              === undefined ? false : gridOptions.enableCellEditOnFocus;
            },
            ...
            editColumnBuilder: function (colDef, col, gridOptions) {
               ...
               */
              colDef.enableCellEdit = colDef.enableCellEdit === undefined
                  ? (gridOptions.enableCellEdit === undefined ?
                      (colDef.type !== 'object') : gridOptions.enableCellEdit)
                  : colDef.enableCellEdit;
               ...
               *  </pre>
               */
              colDef.cellEditableCondition = colDef.cellEditableCondition
              === undefined ? gridOptions.cellEditableCondition
                  : colDef.cellEditableCondition;
              ...
              if (colDef.enableCellEdit) {
                colDef.editableCellTemplate = colDef.editableCellTemplate
                    || gridOptions.editableCellTemplate || 'ui-grid/cellEditor';
               ...
               */
              //enableCellEditOnFocus can only be used if cellnav module is used
              colDef.enableCellEditOnFocus = colDef.enableCellEditOnFocus
              === undefined ? gridOptions.enableCellEditOnFocus
                  : colDef.enableCellEditOnFocus;
   ...
   */
  module.directive('uiGridEdit',
      ['gridUtil', 'uiGridEditService', function (gridUtil, uiGridEditService) {
        return {
          replace: true,
          priority: 0,
          require: '^uiGrid',
  ...
  module.directive('uiGridCell',
      ['$compile', '$injector', '$timeout', 'uiGridConstants',
        'uiGridEditConstants', 'gridUtil', '$parse', 'uiGridEditService',
        function ($compile, $injector, $timeout, uiGridConstants,
            uiGridEditConstants, gridUtil, $parse, uiGridEditService) {
          ...
          return {
            ...
            require: '?^uiGrid',
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
              if (!$scope.col.colDef.enableCellEdit || $scope.row.enableCellEdit
                  === false) {
                return;
              }
              ...
              try {
                var uiGridCellNavConstants = $injector.get(
                    'uiGridCellNavConstants');

                if ($scope.col.colDef.enableCellEditOnFocus) {
                  $scope.$on(uiGridCellNavConstants.CELL_NAV_EVENT,
                      function (evt, rowCol) {
                        if (rowCol.row === $scope.row && rowCol.col
                            === $scope.col) {
                          beginEdit();
                        } else {
                          endEdit();
              ...
              function beginEdit() {
                ...
                if ($scope.grid.api.cellNav) {
                  $scope.grid.api.cellNav.scrollToIfNecessary($scope,
                      $scope.row, $scope.col);
                }
                ...
                html = html.replace(uiGridConstants.MODEL_COL_FIELD,
                    $scope.row.getQualifiedColField($scope.col));

                var optionFilter = $scope.col.colDef.editDropdownFilter ? '|'
                    + $scope.col.colDef.editDropdownFilter : '';
                html = html.replace(uiGridConstants.CUSTOM_FILTERS,
                    optionFilter);
                ...
                if (editDropdownRowEntityOptionsArrayPath) {
                  $scope.editDropdownOptionsArray = resolveObjectFromPath(
                      $scope.row.entity, editDropdownRowEntityOptionsArrayPath);
                } else {
                  $scope.editDropdownOptionsArray = $scope.col.colDef.editDropdownOptionsArray;
                }
                $scope.editDropdownIdLabel = $scope.col.colDef.editDropdownIdLabel
                    ? $scope.col.colDef.editDropdownIdLabel : 'id';
                $scope.editDropdownValueLabel = $scope.col.colDef.editDropdownValueLabel
                    ? $scope.col.colDef.editDropdownValueLabel : 'value';
                ...
                $scope.$apply(function () {
                  ...
                  isFocusedBeforeEdit = gridCellContentsEl.hasClass(
                      'ui-grid-cell-focus');
                  gridCellContentsEl.addClass('ui-grid-cell-contents-hidden');
                });

                //stop editing when grid is scrolled
                var deregOnGridScroll = $scope.col.grid.api.core.on.scrollEvent(
                    $scope, function () {
                      endEdit(true);
                      $scope.grid.api.edit.raise.afterCellEdit(
                          $scope.row.entity, $scope.col.colDef,
                          cellModel($scope), origCellValue);
                      deregOnGridScroll();
                      deregOnEndCellEdit();
                ...
                //end editing
                var deregOnEndCellEdit = $scope.$on(
                    uiGridEditConstants.events.END_CELL_EDIT,
                    function (evt, retainFocus) {
                      endEdit(retainFocus);
                      $scope.grid.api.edit.raise.afterCellEdit(
                          $scope.row.entity, $scope.col.colDef,
                          cellModel($scope), origCellValue);
                ...
                //cancel editing
                var deregOnCancelCellEdit = $scope.$on(
                    uiGridEditConstants.events.CANCEL_CELL_EDIT, function () {
                      cancelEdit();
                      deregOnCancelCellEdit();
                      deregOnGridScroll();
                      deregOnEndCellEdit();
                ...
                $scope.grid.api.edit.raise.beginCellEdit($scope.row.entity,
                    $scope.col.colDef);
              }
              ...
              function cancelEdit() {
                ...
                $scope.grid.api.edit.raise.cancelCellEdit($scope.row.entity,
                    $scope.col.colDef);
                endEdit(true);
              }
              ...
              return {
                ...
                post: function ($scope, $elm, $attrs, controllers) {
                  ...
                  $elm.on('keydown', function (evt) {
                    ...
                    else if (uiGridCtrl && uiGridCtrl.hasOwnProperty('cellNav')
                        && renderContainerCtrl) {
  ...
  module.directive('uiGridEditor', ['$filter', function ($filter) {
    ...
    return {
      ...
      link: function (scope, element, attrs, ngModel) {
        ...
        if (angular.version.minor === 2 && attrs.type && attrs.type === 'date'
            && ngModel) {

          ngModel.$formatters.push(function (modelValue) {
            ngModel.$setValidity(null,
                (!modelValue || !isNaN(modelValue.getTime())));
              ...
              return {
                ...
                post: function ($scope, $elm, $attrs) {
                  ...
                  $scope.$on(uiGridEditConstants.events.BEGIN_CELL_EDIT,
                      function () {
                        $elm[0].focus();
                        $elm[0].style.width = ($elm[0].parentElement.offsetWidth
                            - 1) + 'px';
                        $elm.on('blur', function (evt) {
                          $scope.stopEdit(evt);
                        });
...
(function () {
   ...
   */
  module.service('uiGridExpandableService',
      ['gridUtil', '$compile', function (gridUtil, $compile) {
        var service = {
          initializeGrid: function (grid) {
             ...
             *  </pre>
             */
            grid.options.enableExpandable = grid.options.enableExpandable
                !== false;
             ...
             *  </pre>
             */
            grid.options.expandableRowHeight = grid.options.expandableRowHeight
                || 150;
             ...
             *  </pre>
             */
            grid.options.expandableRowHeaderWidth = grid.options.expandableRowHeaderWidth
                || 40;
             ...
             *  </pre>
             */
            if (grid.options.enableExpandable
                && !grid.options.expandableRowTemplate) {
              gridUtil.logError(
          ...
          toggleRowExpansion: function (grid, row) {
            ...
            if (row.isExpanded) {
              row.height = row.grid.options.rowHeight
                  + grid.options.expandableRowHeight;
            } else {
              row.height = row.grid.options.rowHeight;
              grid.expandable.expandedAll = false;
            }
          ...
          expandAllRows: function (grid, $scope) {
            angular.forEach(grid.renderContainers.body.visibleRowCache,
                function (row) {
                  if (!row.isExpanded) {
                    service.toggleRowExpansion(grid, row);
                  }
   ...
   *  </pre>
   */
  module.directive('uiGridExpandable',
      ['uiGridExpandableService', '$templateCache',
                ...
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  if (uiGridCtrl.grid.options.enableExpandableRowHeader
                    ...
                    var expandableRowHeaderColDef = {
                      ...
                      width: uiGridCtrl.grid.options.expandableRowHeaderWidth
                          || 40
                    };
                    expandableRowHeaderColDef.cellTemplate = $templateCache.get(
                        'ui-grid/expandableRowHeader');
                    expandableRowHeaderColDef.headerCellTemplate = $templateCache.get(
                        'ui-grid/expandableTopRowHeader');
                    uiGridCtrl.grid.addRowHeaderColumn(
        ...
        compile: function () {
          return {
            pre: function ($scope, $elm, $attrs, uiGridCtrl) {
              ...
              uiGridCtrl.grid.api.core.on.renderingComplete($scope,
                  function () {
  ...
  module.directive('uiGridExpandableRow',
      ['uiGridExpandableService', '$timeout', '$compile', 'uiGridConstants',
        'gridUtil', '$interval', '$log',
        function (uiGridExpandableService, $timeout, $compile, uiGridConstants,
            gridUtil, $interval, $log) {
              ...
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  gridUtil.getTemplate(
                      $scope.grid.options.expandableRowTemplate).then(
                      function (template) {
              ...
              return {
                pre: function ($scope, $elm, $attrs, controllers) {
                  ...
                  $scope.expandableRow.shouldRenderExpand = function () {
                    var ret = $scope.colContainer.name === 'body'
                        && $scope.grid.options.enableExpandable !== false
                        && $scope.row.isExpanded
                        && (!$scope.grid.isScrollingVertically
                            || $scope.row.expandedRendered);
                    return ret;
                  };

                  $scope.expandableRow.shouldRenderFiller = function () {
                    var ret = $scope.row.isExpanded && ($scope.colContainer.name
                        !== 'body' || ($scope.grid.isScrollingVertically
                            && !$scope.row.expandedRendered));
                    return ret;
                  };
                  ...
                  function updateRowContainerWidth() {
                    ...
                    return '.grid' + grid.id + ' .ui-grid-pinned-container-'
                        + $scope.colContainer.name + ', .grid' + grid.id +
                        ' .ui-grid-pinned-container-' + $scope.colContainer.name
                        + ' .ui-grid-render-container-'
                        + $scope.colContainer.name +
                        ' .ui-grid-viewport .ui-grid-canvas .ui-grid-row { width: '
                        + colWidth + 'px; }';
                  }
        ...
        function ($compile, gridUtil, $templateCache) {
          return {
            ...
            compile: function ($elm, $attrs) {
              ...
              var expandedRowFillerElement = $templateCache.get(
                  'ui-grid/expandableScrollFiller');
              var expandedRowElement = $templateCache.get(
...
(function () {
   ...
   */
  module.service('uiGridExporterService',
      ['$q', 'uiGridExporterConstants', 'uiGridSelectionConstants', 'gridUtil',
        '$compile', '$interval', 'i18nService',
        function ($q, uiGridExporterConstants, uiGridSelectionConstants,
            gridUtil, $compile, $interval, i18nService) {
          ...
          var service = {
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               * <br/>Defaults to false
               */
              gridOptions.exporterSuppressMenu = gridOptions.exporterSuppressMenu
                  === true;
              /**
               * @ngdoc object
               ...
               */
              gridOptions.exporterMenuLabel = gridOptions.exporterMenuLabel
                  ? gridOptions.exporterMenuLabel : 'Export';
              /**
               * @ngdoc object
               ...
               * </pre>
               */
              gridOptions.exporterSuppressColumns = gridOptions.exporterSuppressColumns
                  ? gridOptions.exporterSuppressColumns : [];
              /**
               * @ngdoc object
               ...
               * <br/>Defaults to ','
               */
              gridOptions.exporterCsvColumnSeparator = gridOptions.exporterCsvColumnSeparator
               ...
               */
              gridOptions.exporterCsvFilename = gridOptions.exporterCsvFilename
                  ? gridOptions.exporterCsvFilename : 'download.csv';
              /**
               * @ngdoc object
               ...
               * </pre>
               */
              gridOptions.exporterPdfDefaultStyle = gridOptions.exporterPdfDefaultStyle
                  ? gridOptions.exporterPdfDefaultStyle : {fontSize: 11};
              /**
               * @ngdoc object
               ...
               * </pre>
               */
              gridOptions.exporterPdfTableStyle = gridOptions.exporterPdfTableStyle
                  ? gridOptions.exporterPdfTableStyle : {margin: [0, 5, 0, 15]};
              /**
               * @ngdoc object
               ...
               * </pre>
               */
              gridOptions.exporterPdfTableHeaderStyle = gridOptions.exporterPdfTableHeaderStyle
                  ? gridOptions.exporterPdfTableHeaderStyle : {
                    bold: true,
                    fontSize: 12,
                    color: 'black'
                  };
               ...
               * </pre>
               */
              gridOptions.exporterPdfHeader = gridOptions.exporterPdfHeader
                  ? gridOptions.exporterPdfHeader : null;
              /**
               * @ngdoc object
               ...
               * </pre>
               */
              gridOptions.exporterPdfFooter = gridOptions.exporterPdfFooter
                  ? gridOptions.exporterPdfFooter : null;
              /**
               * @ngdoc object
               ...
               */
              gridOptions.exporterPdfOrientation = gridOptions.exporterPdfOrientation
               ...
               * <br/>Defaults to A4
               */
              gridOptions.exporterPdfPageSize = gridOptions.exporterPdfPageSize
               ...
               */
              gridOptions.exporterPdfMaxGridWidth = gridOptions.exporterPdfMaxGridWidth
                  ? gridOptions.exporterPdfMaxGridWidth : 720;
              /**
               * @ngdoc object
               ...
               */
              gridOptions.exporterMenuCsv = gridOptions.exporterMenuCsv
              !== undefined ? gridOptions.exporterMenuCsv : true;
               ...
               */
              gridOptions.exporterMenuPdf = gridOptions.exporterMenuPdf
              !== undefined ? gridOptions.exporterMenuPdf : true;
               ...
               * </pre>
               */
              gridOptions.exporterPdfCustomFormatter = (gridOptions.exporterPdfCustomFormatter
                  && typeof (gridOptions.exporterPdfCustomFormatter)
                  === 'function') ? gridOptions.exporterPdfCustomFormatter
                  : function (docDef) {
                    return docDef;
                  };
               ...
               * </pre>
               */
              gridOptions.exporterHeaderFilterUseName = gridOptions.exporterHeaderFilterUseName
                  === true;
               ...
               * </pre>
               */
              gridOptions.exporterFieldCallback = gridOptions.exporterFieldCallback
                  ? gridOptions.exporterFieldCallback : function (grid, row,
                      col, value) {
                    return value;
                  };
            },
            ...
            addToMenu: function (grid) {
              grid.api.core.addToGridMenu(grid, [
                ...
                {
                  title: i18nService.getSafeText(
                      'gridMenu.exporterVisibleAsCsv'),
                  action: function ($event) {
                    this.grid.api.exporter.csvExport(
                ...
                {
                  title: i18nService.getSafeText(
                      'gridMenu.exporterSelectedAsCsv'),
                  action: function ($event) {
                    this.grid.api.exporter.csvExport(
                        uiGridExporterConstants.SELECTED,
                        uiGridExporterConstants.VISIBLE);
                  ...
                  shown: function () {
                    return this.grid.options.exporterMenuCsv &&
                        (this.grid.api.selection
                            && this.grid.api.selection.getSelectedRows().length
                            > 0);
                  }
                },
                {
                  ...
                  action: function ($event) {
                    this.grid.api.exporter.pdfExport(
                        uiGridExporterConstants.ALL,
                        uiGridExporterConstants.ALL);
                  },
                  shown: function () {
                ...
                {
                  title: i18nService.getSafeText(
                      'gridMenu.exporterSelectedAsPdf'),
                  action: function ($event) {
                    this.grid.api.exporter.pdfExport(
                        uiGridExporterConstants.SELECTED,
                        uiGridExporterConstants.VISIBLE);
            ...
            getColumnHeaders: function (grid, colTypes) {
              ...
              angular.forEach(grid.columns, function (gridCol, index) {
                if ((gridCol.visible || colTypes
                    === uiGridExporterConstants.ALL) &&
                    gridCol.colDef.exporterSuppressExport !== true &&
                    grid.options.exporterSuppressColumns.indexOf(gridCol.name)
                    === -1) {
                  headers.push({
                    name: gridCol.field,
                    displayName: grid.options.exporterHeaderFilter
                        ? (grid.options.exporterHeaderFilterUseName
                            ? grid.options.exporterHeaderFilter(gridCol.name)
                            : grid.options.exporterHeaderFilter(
                                gridCol.displayName)) : gridCol.displayName,
                    width: gridCol.drawnWidth ? gridCol.drawnWidth
                        : gridCol.width,
                ...
                if (row.exporterEnableExporting !== false) {
                  ...
                  angular.forEach(grid.columns, function (gridCol, index) {
                    if ((gridCol.visible || colTypes
                        === uiGridExporterConstants.ALL) &&
                        gridCol.colDef.exporterSuppressExport !== true &&
                        grid.options.exporterSuppressColumns.indexOf(
                            gridCol.name) === -1) {
                      var extractedField = {
                        value: grid.options.exporterFieldCallback(grid, row,
                            gridCol, grid.getCellValue(row, gridCol))
                      };
            ...
            downloadFile: function (fileName, csvContent) {
              ...
              if ('download' in a) {
                ...
                rawFile = 'data:' + strMimeType + ',' + encodeURIComponent(
                    csvContent);
            ...
            pdfExport: function (grid, rowTypes, colTypes) {
              var exportColumnHeaders = this.getColumnHeaders(grid, colTypes);
              var exportData = this.getData(grid, rowTypes, colTypes);
              var docDefinition = this.prepareAsPdf(grid, exportColumnHeaders,
                  exportData);
            ...
            prepareAsPdf: function (grid, exportColumnHeaders, exportData) {
              var headerWidths = this.calculatePdfHeaderWidths(grid,
                  exportColumnHeaders);
            ...
            calculatePdfHeaderWidths: function (grid, exportHeaders) {
              ...
              angular.forEach(exportHeaders, function (value) {
                ...
                if (typeof (value.width) === 'string' && value.width.match(
                    /(\d)*%/)) {
              ...
              return exportHeaders.map(function (header) {
                return header.width === '*' ? header.width : header.width
                    * grid.options.exporterPdfMaxGridWidth / gridWidth;
              });
            ...
            formatFieldAsPdfString: function (field) {
              ...
              if (field.value == null) { // we want to catch anything null-ish, hence just == not ===
                ...
                returnVal = JSON.stringify(field.value).replace(/^"/,
                    '').replace(/"$/, '');
              }
   ...
   */
  module.directive('uiGridExporter',
      ['uiGridExporterConstants', 'uiGridExporterService', 'gridUtil',
        '$compile',
        function (uiGridExporterConstants, uiGridExporterService, gridUtil,
            $compile) {
          return {
            replace: true,
            priority: 0,
...
(function () {
   ...
   */
  module.service('uiGridImporterService',
      ['$q', 'uiGridConstants', 'uiGridImporterConstants', 'gridUtil',
        '$compile', '$interval', 'i18nService', '$window',
        function ($q, uiGridConstants, uiGridImporterConstants, gridUtil,
            $compile, $interval, i18nService, $window) {
          ...
          var service = {
            ...
            initializeGrid: function ($scope, grid) {
              ...
              if (grid.options.enableImporter
                  && grid.options.importerShowMenu) {
                if (grid.api.core.addToGridMenu) {
                  service.addToMenu(grid);
                } else {
                  // order of registration is not guaranteed, register in a little while
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               *
               */
              if (gridOptions.enableImporter || gridOptions.enableImporter
                  === undefined) {
                if (!($window.hasOwnProperty('File') && $window.hasOwnProperty(
                    'FileReader') && $window.hasOwnProperty('FileList')
                    && $window.hasOwnProperty('Blob'))) {
                  gridUtil.logError(
               ...
               *
               */
              gridOptions.importerProcessHeaders = gridOptions.importerProcessHeaders
                  || service.processHeaders;
               ...
               *
               */
              gridOptions.importerHeaderFilter = gridOptions.importerHeaderFilter
               ...
               *
               */
              if (!gridOptions.importerErrorCallback
                  || typeof (gridOptions.importerErrorCallback)
                  !== 'function') {
               ...
               *
               */
              if (gridOptions.enableImporter === true
                  && !gridOptions.importerDataAddCallback) {
                gridUtil.logError(
               ...
               * </pre>
               */
              gridOptions.importerObjectCallback = gridOptions.importerObjectCallback
            ...
            importJsonClosure: function (grid) {
              return function (importFile) {
                ...
                angular.forEach(service.parseJson(grid, importFile),
                    function (value, index) {
                      newObject = service.newObject(grid);
                      angular.extend(newObject, value);
                      newObject = grid.options.importerObjectCallback(grid,
                          newObject);
            ...
            parseJson: function (grid, importFile) {
              ...
              try {
                ...
                service.alertError(grid, 'importer.invalidJson',
                    'File could not be processed, is it valid json? Content was: ',
                    importFile.target.result);
                return;
              }

              if (!Array.isArray(loadedObjects)) {
                service.alertError(grid, 'importer.jsonNotarray',
            ...
            importCsvClosure: function (grid) {
              return function (importFile) {
                ...
                if (!importArray || importArray.length < 1) {
                  service.alertError(grid, 'importer.invalidCsv',
                ...
                if (!newObjects || newObjects.length === 0) {
                  service.alertError(grid, 'importer.noObjects',
            ...
            createCsvObjects: function (grid, importArray) {
              ...
              var headerMapping = grid.options.importerProcessHeaders(grid,
                  importArray.shift());
              if (!headerMapping || headerMapping.length === 0) {
                service.alertError(grid, 'importer.noHeaders',
            ...
            processHeaders: function (grid, headerRow) {
              ...
              if (!grid.options.columnDefs || grid.options.columnDefs.length
                ...
                var lookupHash = service.flattenColumnDefs(grid,
            ...
            flattenColumnDefs: function (grid, columnDefs) {
              ...
              angular.forEach(columnDefs, function (columnDef, index) {
                if (columnDef.name) {
                  flattenedHash[columnDef.name] = columnDef.field
                      || columnDef.name;
                  flattenedHash[columnDef.name.toLowerCase()] = columnDef.field
                      || columnDef.name;
                }

                if (columnDef.field) {
                  flattenedHash[columnDef.field] = columnDef.field
                      || columnDef.name;
                  flattenedHash[columnDef.field.toLowerCase()] = columnDef.field
                      || columnDef.name;
                }

                if (columnDef.displayName) {
                  flattenedHash[columnDef.displayName] = columnDef.field
                      || columnDef.name;
                  flattenedHash[columnDef.displayName.toLowerCase()] = columnDef.field
                      || columnDef.name;
                }

                if (columnDef.displayName
                    && grid.options.importerHeaderFilter) {
                  flattenedHash[grid.options.importerHeaderFilter(
                      columnDef.displayName)] = columnDef.field
                      || columnDef.name;
                  flattenedHash[grid.options.importerHeaderFilter(
                      columnDef.displayName).toLowerCase()] = columnDef.field
                      || columnDef.name;
                }
              });
            ...
            addObjects: function (grid, newObjects, $scope) {
              if (grid.api.rowEdit) {
                var dataChangeDereg = grid.registerDataChangeCallback(
                    function () {
            ...
            newObject: function (grid) {
              if (typeof (grid.options) !== "undefined"
                  && typeof (grid.options.importerNewObject) !== "undefined") {
             ...
             * the column definitions
             */
            alertError: function (grid, alertI18nToken, consoleMessage,
                context) {
              if (grid.options.importerErrorCallback) {
                grid.options.importerErrorCallback(grid, alertI18nToken,
   ...
   *
   */
  module.directive('uiGridImporter',
      ['uiGridImporterConstants', 'uiGridImporterService', 'gridUtil',
        '$compile',
        function (uiGridImporterConstants, uiGridImporterService, gridUtil,
            $compile) {
          return {
            replace: true,
            priority: 0,
   ...
   *
   */
  module.directive('uiGridImporterMenuItem',
      ['uiGridImporterConstants', 'uiGridImporterService', 'gridUtil',
        '$compile',
        function (uiGridImporterConstants, uiGridImporterService, gridUtil,
            $compile) {
          return {
            ...
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
              ...
              var fileChooser = $elm[0].querySelectorAll(
                  '.ui-grid-importer-file-chooser');
              var grid = uiGridCtrl.grid;
              ...
              if (fileChooser.length !== 1) {
                ...
                fileChooser[0].addEventListener('change', handleFileSelect,
                    false);  // TODO: why the false on the end?  Google  
              }
            }
          };
        }
...
(function () {
  ...
  module.service('uiGridInfiniteScrollService',
      ['gridUtil', '$compile', '$timeout', 'uiGridConstants',
        function (gridUtil, $compile, $timeout, uiGridConstants) {
          ...
          var service = {
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               *  <br/>Defaults to true
               */
              gridOptions.enableInfiniteScroll = gridOptions.enableInfiniteScroll
                  !== false;
            },
            ...
            checkScroll: function (grid, scrollTop) {
              ...
              var infiniteScrollPercentage = grid.options.infiniteScrollPercentage
                  ? grid.options.infiniteScrollPercentage : 20;

              if (!grid.options.loadTimout && scrollTop
  ...
  module.directive('uiGridViewport',
      ['$compile', 'gridUtil', 'uiGridInfiniteScrollService', 'uiGridConstants',
        function ($compile, gridUtil, uiGridInfiniteScrollService,
            uiGridConstants) {
          return {
            priority: -200,
            scope: false,
            link: function ($scope, $elm, $attr) {
              if ($scope.grid.options.enableInfiniteScroll) {
                $scope.grid.api.core.on.scrollEvent($scope, function (args) {
                  ...
                  if (args.y && (args.source
                      !== 'ui.grid.adjustInfiniteScrollPosition')) {
                    var percentage = 100 - (args.y.percentage * 100);
                    if ($scope.grid.scrollDirection
                        === uiGridConstants.scrollDirection.UP) {
                      percentage = (args.y.percentage * 100);
                    }
                    uiGridInfiniteScrollService.checkScroll($scope.grid,
                        percentage);
                  }
                });
              }
...
(function () {
   ...
   */
  module.service('uiGridMoveColumnService',
      ['$q', '$timeout', '$log', 'ScrollEvent', 'uiGridConstants',
        function ($q, $timeout, $log, ScrollEvent, uiGridConstants) {
          ...
          var service = {
            ...
            registerPublicApi: function (grid) {
              ...
              var publicApi = {
                events: {
                   ...
                   */
                  colMovable: {
                    columnPositionChanged: function (colDef, originalPosition,
                        newPosition) {
                    }
                  }
                },
                methods: {
                  ...
                  colMovable: {
                    moveColumn: function (originalPosition, finalPosition) {
                      ...
                      if (!angular.isNumber(originalPosition)
                          || !angular.isNumber(finalPosition)) {
                        console.log(
                            'Please provide valid values for originalPosition and finalPosition');
                        return;
                      }
                      var nonMovableColumns = 0;
                      for (var i = 0; i < columns.length; i++) {
                        if ((angular.isDefined(columns[i].colDef.visible)
                            && columns[i].colDef.visible === false)
                            || columns[i].isRowHeader === true) {
                          nonMovableColumns++;
                        }
                      }
                      if (originalPosition >= (columns.length
                          - nonMovableColumns) || finalPosition
                          >= (columns.length - nonMovableColumns)) {
                        console.log(
                            'Invalid values for originalPosition, finalPosition');
                        return;
                      }
                      var findPositionForRenderIndex = function (index) {
                        var position = index;
                        for (var i = 0; i <= position; i++) {
                          if (angular.isDefined(columns[i])
                              && ((angular.isDefined(columns[i].colDef.visible)
                                  && columns[i].colDef.visible === false)
                                  || columns[i].isRowHeader === true)) {
                            position++;
                          }
                      ...
                      self.redrawColumnAtPosition(grid,
                          findPositionForRenderIndex(originalPosition),
                          findPositionForRenderIndex(finalPosition));
                    }
                  }
                }
              };
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               */
              gridOptions.enableColumnMoving = gridOptions.enableColumnMoving
                  !== false;
            },
            movableColumnBuilder: function (colDef, col, gridOptions) {
               ...
               */
              colDef.enableColumnMoving = colDef.enableColumnMoving
              === undefined ? gridOptions.enableColumnMoving
                  : colDef.enableColumnMoving;
              return $q.all(promises);
            },
            redrawColumnAtPosition: function (grid, originalPosition,
                newPosition) {
              ...
              if (originalColumn.colDef.enableColumnMoving) {
                ...
                $timeout(function () {
                  grid.refresh();
                  grid.api.colMovable.raise.columnPositionChanged(
                      originalColumn.colDef, originalPosition, newPosition);
                  grid.api.core.notifyDataChange(
                      uiGridConstants.dataChange.COLUMN);
                });
              }
            }
  ...
  module.directive('uiGridMoveColumns',
      ['uiGridMoveColumnService', function (uiGridMoveColumnService) {
        return {
          replace: true,
          priority: 0,
          require: '^uiGrid',
   ...
   *    - mouseup
   */
  module.directive('uiGridHeaderCell',
      ['$q', 'gridUtil', 'uiGridMoveColumnService', '$document', '$log',
        'uiGridConstants', 'ScrollEvent',
              ...
              return {
                post: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  if ($scope.col.colDef.enableColumnMoving) {
                    ...
                    $scope.$on(uiGridConstants.events.COLUMN_HEADER_CLICK,
                        function (event, args) {

                          if (args.columnName === $scope.col.colDef.name
                              && !$scope.col.renderContainer) {
                            ...
                            if (evt.target.className
                              ...
                              var rightMoveLimit = gridLeft
                                  + $scope.grid.getViewportWidth();// - $scope.grid.verticalScrollbarWidth;
                              ...
                              var cloneElement = function () {
                                ...
                                movingElementStyles.left = (elmLeft - gridLeft)
                                    + 'px';
                                var gridRight = $scope.grid.element[0].getBoundingClientRect().right;
                                var elmRight = $elm[0].getBoundingClientRect().right;
                                if (elmRight > gridRight) {
                                  reducedWidth = $scope.col.drawnWidth
                                      + (gridRight - elmRight);
                                  movingElementStyles.width = reducedWidth
                                      + 'px';
                                }
                              ...
                              var moveElement = function (changeValue) {
                                ...
                                for (var i = 0; i < columns.length; i++) {
                                  if (angular.isUndefined(
                                      columns[i].colDef.visible)
                                      || columns[i].colDef.visible === true) {
                                    totalColumnWidth += columns[i].drawnWidth
                                        || columns[i].width
                                        || columns[i].colDef.width;
                                  }
                                }
                                ...
                                if (gridUtil.detectBrowser() === 'ie') {
                                  ...
                                  newElementLeft = currentElmLeft - gridLeft
                                      + changeValue;
                                }
                                newElementLeft = newElementLeft < rightMoveLimit
                                    ? newElementLeft : rightMoveLimit;

                                //Update css of moving column to adjust to new left value or fire scroll in case column has reached edge of grid
                                if ((currentElmLeft >= gridLeft || changeValue
                                    > 0) && (currentElmRight <= rightMoveLimit
                                    || changeValue < 0)) {
                                  movingElm.css({
                                    visibility: 'visible',
                                    'left': newElementLeft + 'px'
                                  });
                                } else if (totalColumnWidth > Math.ceil(
                                    uiGridCtrl.grid.gridWidth)) {
                                  changeValue *= 8;
                                  var scrollEvent = new ScrollEvent(
                                      $scope.col.grid, null, null,
                                ...
                                for (var il = 0; il < columns.length; il++) {
                                  if (angular.isUndefined(
                                      columns[il].colDef.visible)
                                      || columns[il].colDef.visible === true) {
                                    if (columns[il].colDef.name
                                        !== $scope.col.colDef.name) {
                                      totalColumnsLeftWidth += columns[il].drawnWidth
                                          || columns[il].width
                                          || columns[il].colDef.width;
                                    } else {
                                      break;
                                    }
                                  }
                                ...
                                if ($scope.newScrollLeft === undefined) {
                                  ...
                                  totalMouseMovement = $scope.newScrollLeft
                                      + newElementLeft - totalColumnsLeftWidth;
                                }
                       ...
                       */
                              ...
                              var mouseUpHandler = function (evt) {
                                  ...
                                  for (var il = columnIndex - 1; il >= 0;
                                    ...
                                    if (angular.isUndefined(
                                      ...
                                      totalColumnsLeftWidth += columns[il].drawnWidth
                                          || columns[il].width
                                          || columns[il].colDef.width;
                                      if (totalColumnsLeftWidth > Math.abs(
                                          totalMouseMovement)) {
                                ...
                                //Case where column should be moved to a position on its right
                                  ...
                                  for (var ir = columnIndex + 1;
                                      ir < columns.length; ir++) {
                                    if (angular.isUndefined(
                                        columns[ir].colDef.visible)
                                        || columns[ir].colDef.visible
                                        === true) {
                                      totalColumnsRightWidth += columns[ir].drawnWidth
                                          || columns[ir].width
                                          || columns[ir].colDef.width;
                                      if (totalColumnsRightWidth
                                          > totalMouseMovement) {
                                        uiGridMoveColumnService.redrawColumnAtPosition
...
(function () {
    ...
    function (gridUtil) {
      var service = {
          ...
          var publicApi = {
            ...
            methods: {
              pagination: {
                 ...
                 */
                getPage: function () {
                  return grid.options.enablePagination
                      ? grid.options.paginationCurrentPage : null;
                },
                /**
                 * @ngdoc method
                 * @name getTotalPages
                ...
                getTotalPages: function () {
                  ...
                  return (grid.options.totalItems === 0) ? 1 : Math.ceil(
                ...
                previousPage: function () {
                  ...
                  grid.options.paginationCurrentPage = Math.max(
                      grid.options.paginationCurrentPage - 1, 1);
                },
                /**
                 * @ngdoc method
                 * @name seek
                ...
                seek: function (page) {
                  ...
                  grid.options.paginationCurrentPage = Math.min(page,
                      publicApi.methods.pagination.getTotalPages());
                }
              }
            }
        ...
        defaultGridOptions: function (gridOptions) {
           ...
           */
          gridOptions.enablePaginationControls = gridOptions.enablePaginationControls
              !== false;
          /**
           * @ngdoc property
           ...
           */
          gridOptions.useExternalPagination = gridOptions.useExternalPagination
              === true;
          /**
           * @ngdoc property
           * @name totalItems
  ...
  module.directive('uiGridPager',
      ['uiGridPaginationService', 'uiGridConstants', 'gridUtil', 'i18nService',
        function (uiGridPaginationService, uiGridConstants, gridUtil,
            i18nService) {
          return {
            priority: -200,
            scope: true,
            ...
            link: function ($scope, $elm, $attr, uiGridCtrl) {
              ...
              $scope.totalItemsLabel = i18nService.getSafeText(
                  'pagination.totalItems');

              var options = uiGridCtrl.grid.options;

              uiGridCtrl.grid.renderContainers.body.registerViewportAdjuster(
                  function (adjustment) {
              ...
              var dataChangeDereg = uiGridCtrl.grid.registerDataChangeCallback(
                  function (grid) {
                    if (!grid.options.useExternalPagination) {
              ...
              var setShowing = function () {
                $scope.showingLow = ((options.paginationCurrentPage - 1)
                    * options.paginationPageSize) + 1;
                $scope.showingHigh = Math.min(
                    options.paginationCurrentPage * options.paginationPageSize,
                    options.totalItems);
              };
              ...
              var deregP = $scope.$watch(
                  ...
                  function (newValues, oldValues) {
                    ...
                    if (!angular.isNumber(options.paginationCurrentPage)
                        || options.paginationCurrentPage < 1) {
                      options.paginationCurrentPage = 1;
                      return;
                    }

                    if (options.totalItems > 0 && options.paginationCurrentPage
                        > $scope.paginationApi.getTotalPages()) {
                      options.paginationCurrentPage = $scope.paginationApi.getTotalPages();
                      return;
                    }

                    setShowing();
                    uiGridPaginationService.onPaginationChanged($scope.grid,
                        options.paginationCurrentPage,
              ...
              $scope.cantPageForward = function () {
                if (options.totalItems > 0) {
                  return options.paginationCurrentPage
                      >= $scope.paginationApi.getTotalPages();
                } else {
...
(function () {
  ...
  module.service('uiGridPinningService',
      ['gridUtil', 'GridRenderContainer', 'i18nService',
        function (gridUtil, GridRenderContainer, i18nService) {
          var service = {
            ...
            pinningColumnBuilder: function (colDef, col, gridOptions) {
               ...
               *  <br/>Defaults to true
               */
              colDef.enablePinning = colDef.enablePinning === undefined
                  ? gridOptions.enablePinning : colDef.enablePinning;
              ...
              var pinColumnLeftAction = {
                ...
                shown: function () {
                  return typeof (this.context.col.renderContainer)
                      === 'undefined' || !this.context.col.renderContainer
              ...
              var removePinAction = {
                ...
                shown: function () {
                  return typeof (this.context.col.renderContainer)
                      !== 'undefined' && this.context.col.renderContainer
                      !== null && this.context.col.renderContainer !== 'body';
                },
                action: function () {
              ...
              if (!gridUtil.arrayContainsObjectWithProperty(col.menuItems,
                  'name', 'ui.grid.pinning.pinLeft')) {
...
(function () {
    ...
    function (gridUtil, $q, $timeout) {
      ...
      var service = {
        defaultGridOptions: function (gridOptions) {
           ...
           *  <br/>Defaults to true
           */
          gridOptions.enableColumnResizing = gridOptions.enableColumnResizing
              !== false;
        ...
        colResizerColumnBuilder: function (colDef, col, gridOptions) {
           ...
           */
          //default to true unless gridOptions or colDef is explicitly false
          colDef.enableColumnResizing = colDef.enableColumnResizing
          === undefined ? gridOptions.enableColumnResizing
              : colDef.enableColumnResizing;
        ...
        findTargetCol: function (col, position, rtlMultiplier) {
          ...
          if (position === 'left') {
            ...
            return renderContainer.visibleColumnCache[colIndex - 1
            * rtlMultiplier];
          } else {
            return col;
          }
  ...
  module.directive('uiGridResizeColumns',
      ['gridUtil', 'uiGridResizeColumnsService',
        function (gridUtil, uiGridResizeColumnsService) {
          return {
            replace: true,
            priority: 0,
            require: '^uiGrid',
            ...
            compile: function () {
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  uiGridResizeColumnsService.defaultGridOptions(
                      uiGridCtrl.grid.options);
                  uiGridCtrl.grid.registerColumnBuilder(
                      uiGridResizeColumnsService.colResizerColumnBuilder);
  ...
  module.directive('uiGridHeaderCell',
      ['gridUtil', '$templateCache', '$compile', '$q',
        'uiGridResizeColumnsService', 'uiGridConstants', '$timeout',
        function (gridUtil, $templateCache, $compile, $q,
            uiGridResizeColumnsService, uiGridConstants, $timeout) {
          return {
              ...
              return {
                post: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  if (grid.options.enableColumnResizing) {
                    var columnResizerElm = $templateCache.get(
                    ...
                    var displayResizers = function () {
                      ...
                      var resizers = $elm[0].getElementsByClassName(
                      ...
                      var otherCol = uiGridResizeColumnsService.findTargetCol(
                          $scope.col, 'left', rtlMultiplier);
                      var renderContainer = $scope.col.getRenderContainer();

                      // Don't append the left resizer if this is the first column or the column to the left of this one has resizing disabled
                      if (otherCol
                          && renderContainer.visibleColumnCache.indexOf(
                              $scope.col) !== 0
                          && otherCol.colDef.enableColumnResizing !== false) {
                        var resizerLeft = angular.element(
                            columnResizerElm).clone();
                      ...
                      if ($scope.col.colDef.enableColumnResizing !== false) {
                        var resizerRight = angular.element(
                            columnResizerElm).clone();
                    ...
                    var dataChangeDereg = grid.registerDataChangeCallback(
                        waitDisplay, [uiGridConstants.dataChange.COLUMN]);
   ...
   */
  module.directive('uiGridColumnResizer',
      ['$document', 'gridUtil', 'uiGridConstants', 'uiGridResizeColumnsService',
        function ($document, gridUtil, uiGridConstants,
            uiGridResizeColumnsService) {
          var resizeOverlay = angular.element(
              '<div class="ui-grid-resize-overlay"></div>');
            ...
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
              ...
              function resizeAroundColumn(col) {
                ...
                renderContainer.visibleColumnCache.forEach(function (column) {
                  ...
                  if (!colDef.width || (angular.isString(colDef.width)
                      && (colDef.width.indexOf('*') !== -1
                          || colDef.width.indexOf('%') !== -1))) {
              ...
              function constrainWidth(col, width) {
                ...
                } else if (col.colDef.maxWidth && newWidth
                    > col.colDef.maxWidth) {
              ...
              function mousemove(event, args) {
                ...
                x = (event.targetTouches ? event.targetTouches[0]
                ...
                var col = uiGridResizeColumnsService.findTargetCol($scope.col,
                    $scope.position, rtlMultiplier);
                ...
                var newWidth = parseInt(col.drawnWidth + xDiff * rtlMultiplier,
                    10);

                // check we're not outside the allowable bounds for this column
                x = x + (constrainWidth(col, newWidth) - newWidth)
                    * rtlMultiplier;
              ...
              function mouseup(event, args) {
                ...
                // Resize the column
                x = (event.changedTouches ? event.changedTouches[0]
                    : event).clientX - gridLeft;
                var xDiff = x - startX;
                ...
                uiGridResizeColumnsService.fireColumnSizeChanged(
                    uiGridCtrl.grid, col.colDef, xDiff);
              ...
              $elm.on(downEvent, function (event, args) {
                ...
                startX = (event.targetTouches ? event.targetTouches[0]
                    : event).clientX - gridLeft;
              ...
              $elm.on('dblclick', function (event, args) {
                ...
                var renderContainerElm = gridUtil.closestElm($elm,
                    '.ui-grid-render-container');

                // Get the cell contents so we measure correctly. For the header cell we have to account for the sort icon and the menu buttons, if present
                var cells = renderContainerElm.querySelectorAll(
                    '.' + uiGridConstants.COL_CLASS_PREFIX + col.uid
                ...
                Array.prototype.forEach.call(cells, function (cell) {
                  ...
                  var menuButton;
                  if (angular.element(cell).parent().hasClass(
                      'ui-grid-header-cell')) {
                    menuButton = angular.element(
                        cell).parent()[0].querySelectorAll(
...
(function () {
  ...
  var module = angular.module('ui.grid.rowEdit',
   ...
   */
  module.service('uiGridRowEditService',
      ['$interval', '$q', 'uiGridConstants', 'uiGridRowEditConstants',
        'gridUtil',
        function ($interval, $q, uiGridConstants, uiGridRowEditConstants,
            gridUtil) {
          ...
          var service = {
            ...
            saveRow: function (grid, gridRow) {
              ...
              return function () {
                ...
                if (gridRow.rowEditSavePromise) {
                  gridRow.rowEditSavePromise.then(
                      self.processSuccessPromise(grid, gridRow),
                      self.processErrorPromise(grid, gridRow));
                } else {
                  gridUtil.logError(
            ...
            navigate: function (newRowCol, oldRowCol) {
              ...
              if (oldRowCol && oldRowCol.row && oldRowCol.row
                  !== newRowCol.row) {
            ...
            considerSetTimer: function (grid, gridRow) {
              ...
              if (gridRow.isDirty && !gridRow.isSaving) {
                if (grid.options.rowEditWaitInterval !== -1) {
                  var waitTime = grid.options.rowEditWaitInterval
                      ? grid.options.rowEditWaitInterval : 2000;
                  gridRow.rowEditSaveTimer = $interval(
                      service.saveRow(grid, gridRow), waitTime, 1);
                }
              }
            },
   ...
   *
   */
  module.directive('uiGridRowEdit',
      ['gridUtil', 'uiGridRowEditService', 'uiGridEditConstants',
        ...
        function ($compile, uiGridConstants, gridUtil, $parse) {
          return {
            ...
            compile: function ($elm, $attrs) {
              ...
              var newNgClass = '';
              if (existingNgClass) {
                newNgClass = existingNgClass.slice(0, -1)
...
(function () {
  ...
  var module = angular.module('ui.grid.saveState',
  ...
  module.service('uiGridSaveStateService',
      ['$q', 'uiGridSaveStateConstants', 'gridUtil', '$compile', '$interval',
        'uiGridConstants',
        function ($q, uiGridSaveStateConstants, gridUtil, $compile, $interval,
            uiGridConstants) {
          ...
          var service = {
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               */
              gridOptions.saveFocus = gridOptions.saveScroll !== true
            ...
            saveScrollFocus: function (grid) {
              ...
              if (grid.options.saveFocus) {
                ...
                if (grid.renderContainers.body.prevRowScrollIndex) {
                  scrollFocus.rowVal = service.getRowVal(grid,
                      grid.renderContainers.body.visibleRowCache[grid.renderContainers.body.prevRowScrollIndex]);
                }
            ...
            saveSelection: function (grid) {
              ...
              var selection = grid.api.selection.getSelectedGridRows().map(
                  function (gridRow) {
            ...
            getRowVal: function (grid, gridRow) {
              ...
              if (grid.options.saveRowIdentity) {
                ...
                rowVal.row = grid.renderContainers.body.visibleRowCache.indexOf(
                    gridRow);
              }
              return rowVal;
            },
              ...
              angular.forEach(columnsState, function (columnState, index) {
                ...
                if (currentCol.length > 0) {
                  ...
                  if (grid.columns[currentIndex].visible !== columnState.visible
                      ||
                      grid.columns[currentIndex].colDef.visible
                      !== columnState.visible) {
                    grid.columns[currentIndex].visible = columnState.visible;
                    grid.columns[currentIndex].colDef.visible = columnState.visible;
                    grid.api.core.raise.columnVisibilityChanged(
                        grid.columns[currentIndex]);
                  }

                  grid.columns[currentIndex].width = columnState.width;

                  if (!angular.equals(grid.columns[currentIndex].sort,
                      columnState.sort &&
                      !(grid.columns[currentIndex].sort === undefined
                          && angular.isEmpty(columnState.sort)))) {
                    grid.columns[currentIndex].sort = angular.copy(
                        columnState.sort);
                    grid.api.core.raise.sortChanged();
                  }

                  if (!angular.equals(grid.columns[currentIndex].filters,
                      columnState.filters)) {
                    grid.columns[currentIndex].filters = angular.copy(
                        columnState.filters);
            ...
            restoreScrollFocus: function (grid, $scope, scrollFocusState) {
              ...
              if (scrollFocusState.rowVal && scrollFocusState.rowVal.row) {
                if (scrollFocusState.rowVal.identity) {
                  row = service.findRowByIdentity(grid,
                      scrollFocusState.rowVal);
                } else {
            ...
            findRowByIdentity: function (grid, rowVal) {
              ...
              var filteredRows = grid.rows.filter(function (gridRow) {
                if (grid.options.saveRowIdentity(gridRow.entity)
                    === rowVal.row) {
                  return true;
                } else {
                  return false;
   ...
   */
  module.directive('uiGridSaveState',
      ['uiGridSaveStateConstants', 'uiGridSaveStateService', 'gridUtil',
        '$compile',
        function (uiGridSaveStateConstants, uiGridSaveStateService, gridUtil,
            $compile) {
          return {
            replace: true,
            priority: 0,
...
(function () {
  ...
  module.service('uiGridSelectionService',
      ['$q', '$templateCache', 'uiGridSelectionConstants', 'gridUtil',
        function ($q, $templateCache, uiGridSelectionConstants, gridUtil) {
          ...
          var service = {
                  ...
                  selection: {
                    ...
                    toggleRowSelection: function (rowEntity, evt) {
                      ...
                      if (row !== null && row.enableSelection !== false) {
                        service.toggleRowSelection(grid, row, evt,
                            grid.options.multiSelect, grid.options.noUnselect);
                      }
                    },
                    /**
                     * @ngdoc function
                    ...
                    selectRow: function (rowEntity, evt) {
                      var row = grid.getRow(rowEntity);
                      if (row !== null && !row.isSelected && row.enableSelection
                          !== false) {
                        service.toggleRowSelection(grid, row, evt,
                            grid.options.multiSelect, grid.options.noUnselect);
                      }
                    ...
                    selectRowByVisibleIndex: function (rowNum, evt) {
                      ...
                      if (row !== null && typeof (row) !== 'undefined'
                          && !row.isSelected && row.enableSelection !== false) {
                        service.toggleRowSelection(grid, row, evt,
                            grid.options.multiSelect, grid.options.noUnselect);
                      }
                    },
                    ...
                    selectAllRows: function (evt) {
                      ...
                      grid.rows.forEach(function (row) {
                        if (!row.isSelected && row.enableSelection !== false) {
                          ...
                          service.decideRaiseSelectionEvent(grid, row,
                              changedRows, evt);
                        }
                      });
                    ...
                    selectAllVisibleRows: function (evt) {
                      ...
                      grid.rows.forEach(function (row) {
                        if (row.visible) {
                          if (!row.isSelected && row.enableSelection
                              !== false) {
                            row.setSelected(true);
                            service.decideRaiseSelectionEvent(grid, row,
                                changedRows, evt);
                    ...
                    getSelectedRows: function () {
                      return service.getSelectedRows(grid).map(
                          function (gridRow) {
            ...
            defaultGridOptions: function (gridOptions) {
               ...
               *  <br/>Defaults to true
               */
              gridOptions.enableRowSelection = gridOptions.enableRowSelection
                  !== false;
              /**
               *  @ngdoc object
               *  @name multiSelect
               ...
               *  <br/>Defaults to false
               */
              gridOptions.modifierKeysToMultiSelect = gridOptions.modifierKeysToMultiSelect
                  === true;
              /**
               *  @ngdoc object
               ...
               *  <br/>Defaults to true
               */
              gridOptions.enableRowHeaderSelection = gridOptions.enableRowHeaderSelection
                  !== false;
              /**
               *  @ngdoc object
               ...
               *  <br/>Defaults to true
               */
              gridOptions.enableSelectAll = gridOptions.enableSelectAll
                  !== false;
              /**
               *  @ngdoc object
               ...
               *  <br/>Defaults to true
               */
              gridOptions.enableSelectionBatchEvent = gridOptions.enableSelectionBatchEvent
                  !== false;
              /**
               *  @ngdoc object
               ...
               *  <br/>Defaults to 30px
               */
              gridOptions.selectionRowHeaderWidth = angular.isDefined(
                  gridOptions.selectionRowHeaderWidth)
                  ? gridOptions.selectionRowHeaderWidth : 30;
               ...
               */
              gridOptions.enableFooterTotalSelected = gridOptions.enableFooterTotalSelected
                  !== false;
              ...
              gridOptions.isRowSelectable = angular.isDefined(
                  gridOptions.isRowSelectable) ? gridOptions.isRowSelectable
                  : angular.noop;
            },
             ...
             */
            toggleRowSelection: function (grid, row, evt, multiSelect,
                noUnselect) {
            ...
            shiftSelect: function (grid, row, evt, multiSelect) {
              ...
              var selectedRows = service.getSelectedRows(grid);
              var fromRow = selectedRows.length > 0
                  ? grid.renderContainers.body.visibleRowCache.indexOf(
                      grid.selection.lastSelectedRow) : 0;
              var toRow = grid.renderContainers.body.visibleRowCache.indexOf(
                  row);
              ...
              for (var i = fromRow; i <= toRow; i++) {
                ...
                if (rowToSelect) {
                  if (!rowToSelect.isSelected && rowToSelect.enableSelection
                      !== false) {
                    rowToSelect.setSelected(true);
                    grid.selection.lastSelectedRow = rowToSelect;
                    service.decideRaiseSelectionEvent(grid, rowToSelect,
                        changedRows, evt);
                  }
                }
              }
   ...
   */
  module.directive('uiGridSelection',
      ['uiGridSelectionConstants', 'uiGridSelectionService', '$templateCache',
        function (uiGridSelectionConstants, uiGridSelectionService,
            $templateCache) {
          return {
            replace: true,
            priority: 0,
            ...
            compile: function () {
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  if (uiGridCtrl.grid.options.isRowSelectable
                      !== angular.noop) {
                    uiGridCtrl.grid.registerRowBuilder(function (row, options) {
                      row.enableSelection = uiGridCtrl.grid.options.isRowSelectable(
                          row);
                    });
                  }
                },
  ...
  module.directive('uiGridSelectionRowHeaderButtons',
      ['$templateCache', 'uiGridSelectionService',
            ...
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
              ...
              $scope.selectButtonClick = function (row, evt) {
                if (evt.shiftKey) {
                  ...
                  uiGridSelectionService.toggleRowSelection(self, row, evt,
                      (self.options.multiSelect
                          && !self.options.modifierKeysToMultiSelect),
  ...
  module.directive('uiGridSelectionSelectAllButtons',
      ['$templateCache', 'uiGridSelectionService',
        function ($templateCache, uiGridSelectionService) {
          return {
            replace: true,
   ...
   */
  module.directive('uiGridViewport',
      ['$compile', 'uiGridConstants', 'uiGridSelectionConstants', 'gridUtil',
        '$parse', 'uiGridSelectionService',
        function ($compile, uiGridConstants, uiGridSelectionConstants, gridUtil,
            $parse, uiGridSelectionService) {
          return {
            priority: -200, // run after default  directive
            scope: false,
          ...
          return {
            ...
            link: function ($scope, $elm, $attrs) {
              ...
              var selectCells = function (evt) {
                if (evt.shiftKey) {
                  uiGridSelectionService.shiftSelect($scope.grid, $scope.row,
                      evt, $scope.grid.options.multiSelect);
                } else if (evt.ctrlKey || evt.metaKey) {
                  uiGridSelectionService.toggleRowSelection($scope.grid,
                      $scope.row, evt, $scope.grid.options.multiSelect,
              ...
              function registerRowSelectionEvents() {
                if ($scope.grid.options.enableRowSelection
                    && !$scope.grid.options.enableRowHeaderSelection) {
              ...
              var dataChangeDereg = $scope.grid.registerDataChangeCallback(
                  function () {
                    if ($scope.grid.options.enableRowSelection
                        ...
                        !$scope.registered) {
                      registerRowSelectionEvents();
                    } else if ((!$scope.grid.options.enableRowSelection
                        || $scope.grid.options.enableRowHeaderSelection) &&
                        $scope.registered) {
  ...
  module.directive('uiGridGridFooter',
      ['$compile', 'uiGridConstants', 'gridUtil',
        function ($compile, uiGridConstants, gridUtil) {
          return {
            restrict: 'EA',
              ...
              return {
                pre: function ($scope, $elm, $attrs, uiGridCtrl) {
                  ...
                  .then(function (contents) {
                    ...
                    angular.element($elm[0].getElementsByClassName(
                        'ui-grid-grid-footer')[0]).append(newElm);
                  });
                },
